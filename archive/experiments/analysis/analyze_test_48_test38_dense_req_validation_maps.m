%% analyze_test_48_test38_dense_req_validation_maps.m
% Test 48: dense REQ validation maps for frozen Test 38 models.
%
% This script recalculates REQ on representative maps only. It never trains
% or modifies the saved Test 38 model bundle. The purpose is to make dense
% diagnostic maps and ROI summaries for seen and deliberately unseen cases.
...
write_config_json(CFG, fullfile(OUT.root_dir,'test48_configuration.json'));
...
writetable(T_all, fullfile(OUT.table_dir,'test48_dense_patch_level_results.csv'));
writetable(T_roi, fullfile(OUT.table_dir,'test48_roi_summary.csv'));
writetable(T_overall, fullfile(OUT.table_dir,'test48_summary_overall.csv'));
writetable(T_by_case, fullfile(OUT.table_dir,'test48_summary_by_case.csv'));
writetable(T_by_regime, fullfile(OUT.table_dir,'test48_summary_by_regime.csv'));
writetable(T_by_frequency, fullfile(OUT.table_dir,'test48_summary_by_frequency.csv'));
writetable(T_by_M, fullfile(OUT.table_dir,'test48_summary_by_M.csv'));
writetable(T_by_roi, fullfile(OUT.table_dir,'test48_summary_by_roi.csv'));
...
save(fullfile(OUT.data_dir,'test48_compact_results.mat'), ...
    'CFG','T_overall','T_by_case','T_by_regime','T_by_frequency','T_by_M', ...
    'T_by_roi','T_roi','T33','-v7.3');
...
mode = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST48_MODE'))));
if mode == "", mode = "quick"; end
assert(ismember(mode, ["quick","full"]), 'ADAPTIVE_REQ_TEST48_MODE must be quick or full.');
CFG = struct();
CFG.Mode = mode;
CFG.QuickMode = mode == "quick";
CFG.ValidateOnly = env_true('ADAPTIVE_REQ_TEST48_VALIDATE_ONLY', false);
CFG.SaveAllMaps = env_true('ADAPTIVE_REQ_TEST48_SAVE_ALL_MAPS', true);
CFG.UseParfor = env_true('ADAPTIVE_REQ_TEST48_USE_PARFOR', false);
CFG.TargetStepM = env_num('ADAPTIVE_REQ_TEST48_TARGET_STEP_M', 0.5e-3);
CFG.CompareTest33 = env_true('ADAPTIVE_REQ_TEST48_COMPARE_TEST33', true);
CFG.Test33Source = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST48_TEST33_SOURCE'))));
if CFG.Test33Source == ""
...
CFG.ModelSource = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST48_MODEL_SOURCE'))));
if CFG.ModelSource == ""
    if CFG.QuickMode || CFG.ValidateOnly
...
CFG.ModelsToEvaluate = ["theory_discrete","q_spectrum_only", ...
...
CFG.REQ.Profile = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST48_REQ_PROFILE'))));
...
switch CFG.REQ.Profile
    ...
    otherwise
        error('Unknown ADAPTIVE_REQ_TEST48_REQ_PROFILE: %s', CFG.REQ.Profile);
end
CFG.REQ.Nbins_auto_oversample = env_num('ADAPTIVE_REQ_TEST48_NBINS_OVERSAMPLE', CFG.REQ.Nbins_auto_oversample);
CFG.REQ.Nbins_min = env_num('ADAPTIVE_REQ_TEST48_NBINS_MIN', CFG.REQ.Nbins_min);
...
if CFG.QuickMode
    OUT.root_dir = fullfile(root_dir,'outputs','test_48_test38_dense_req_validation_maps','quick');
else
    OUT.root_dir = fullfile(root_dir,'outputs','test_48_test38_dense_req_validation_maps');
end
if CFG.ValidateOnly
...
T_pred = apply_models(F, MODELS, BASE_FEATURES, "test48_dense_validation", CFG.ModelsToEvaluate);
end
...
n = height(F);
F.dataset = repmat("test48_dense_validation", n, 1);
...
if exist(out_dir,'dir') ~= 7, mkdir(out_dir); end
export_fig(fig, fullfile(out_dir, "test48_roi_overlay__" + sanitize(key) + ".png"));
end
