...
% Test 38: velocity/field-diverse clean spectral q training.
%
% Trains a v2 version of the clean Test 35 spectral-composition q models,
% now with varied material speeds, multiple directional angles, random
% diffuse 2D/3D seeds, and partially diffuse fields with different source
% counts. No frozen Local/Hybrid/confidence/correction maps are predictors.
%
% Runtime controls:
%   ADAPTIVE_REQ_TEST38_MODE                  = quick | medium | full
%   ADAPTIVE_REQ_TEST38_VALIDATE_ONLY         = true | false
%   ADAPTIVE_REQ_TEST38_SAVE_ALL_MAPS         = true | false
%   ADAPTIVE_REQ_TEST38_USE_PARFOR            = true | false
%   ADAPTIVE_REQ_TEST38_USE_PARALLEL_TRAINING = true | false
%   ADAPTIVE_REQ_TEST38_OUTPUT_NAME           = optional output folder name
%   ADAPTIVE_REQ_TEST38_FREQUENCIES_HZ        = optional numeric list
%   ADAPTIVE_REQ_TEST38_M_LIST                = optional numeric list
...
write_config_json(CFG, fullfile(OUT.root_dir,'test38_configuration.json'));

fprintf('\nTest 38: velocity/field-diverse clean q training\n');
...
fprintf('Training clean spectral q models. No previous q/confidence/correction predictors.\n');
fprintf('At least one source is forced/aligned in every regime.\n');

if CFG.ValidateOnly
    validate_test38(root_dir, CFG);
    fprintf('Test 38 validation-only checks passed.\n');
    return;
end
...
dataset_file = fullfile(OUT.data_dir, 'test38_velocity_field_diverse_dataset.mat');
if exist(dataset_file,'file') == 2
    ...
    if isfield(S,'CFG_saved') && cache_signature(S.CFG_saved) == cache_signature(CFG)
        T_train = S.T_train;
        fprintf('Reused Test 38 dataset: %d patches.\n', height(T_train));
    else
        fprintf('Ignoring incompatible Test 38 dataset cache.\n');
...
assert(~isempty(T_train), 'Test 38 dataset is empty.');
...
%% Train same model family as Test 35

MODELS = train_models(T_model_train, true(height(T_model_train),1), BASE_FEATURES, CFG);
clear T_model_train;
T_pred = apply_models(T_model_eval, MODELS, BASE_FEATURES, "synthetic_v2");
clear T_model_eval;
...
writetable(remove_cell_columns(T_pred), fullfile(OUT.table_dir, ...
    'test38_patch_level_predictions.csv'));
writetable(T_comp, fullfile(OUT.table_dir, 'test38_composition_metrics.csv'));
writetable(T_overall, fullfile(OUT.table_dir, 'test38_summary_overall.csv'));
writetable(T_by_case, fullfile(OUT.table_dir, 'test38_summary_by_case.csv'));
writetable(T_by_family, fullfile(OUT.table_dir, 'test38_summary_by_family.csv'));
writetable(T_by_regime, fullfile(OUT.table_dir, 'test38_summary_by_regime.csv'));
writetable(T_by_frequency, fullfile(OUT.table_dir, 'test38_summary_by_frequency.csv'));
writetable(T_by_M, fullfile(OUT.table_dir, 'test38_summary_by_M.csv'));
writetable(T_by_purity, fullfile(OUT.table_dir, 'test38_summary_by_purity_bin.csv'));
writetable(T_fail, fullfile(OUT.table_dir, 'test38_worst_conditions.csv'));

MODEL_BUNDLE = struct('MODELS', MODELS, 'BASE_FEATURES', BASE_FEATURES, ...
    'CFG', CFG, 'policy', "Test38 clean velocity/field-diverse spectral q v2");
save(fullfile(OUT.model_dir, 'test38_velocity_field_diverse_q_models.mat'), ...
    'MODEL_BUNDLE', '-v7.3');
save(fullfile(OUT.data_dir,'test38_results.mat'), ...
    'T_pred','T_held','T_comp','T_overall','T_by_case','T_by_family', ...
    'T_by_regime','T_by_frequency','T_by_M','T_by_purity','T_fail','CFG','-v7.3');
...
safe_plot(@() plot_heatmap_table(T_by_case, "case_id", OUT, 'test38_mape_by_case.png'), 'case heatmap');
safe_plot(@() plot_heatmap_table(T_by_regime, "field_regime_ood", OUT, 'test38_mape_by_regime.png'), 'regime heatmap');
safe_plot(@() plot_heatmap_table(T_by_frequency, "f0", OUT, 'test38_mape_by_frequency.png'), 'frequency heatmap');
safe_plot(@() plot_heatmap_table(T_by_M, "M", OUT, 'test38_mape_by_M.png'), 'M heatmap');
...
assert(ismember(mode, ["quick","medium","full"]), ...
    'ADAPTIVE_REQ_TEST38_MODE must be quick, medium, or full.');
...
CFG.ValidateOnly = env_true('ADAPTIVE_REQ_TEST38_VALIDATE_ONLY', false);
CFG.SaveAllMaps = env_true('ADAPTIVE_REQ_TEST38_SAVE_ALL_MAPS', true);
CFG.RandomSeed = 38001;
CFG.DxList = env_number_list('ADAPTIVE_REQ_TEST38_DX_LIST_MM', 0.2) * 1e-3;
CFG.dx = CFG.DxList(1);
CFG.dz = CFG.dx;
CFG.Lx = 0.05;
CFG.Lz = 0.05;
CFG.TargetStepM = env_number('ADAPTIVE_REQ_TEST38_TARGET_STEP_M', 1.0e-3);
CFG.cs_guess = 3;
CFG.REQ.Nbins = 'auto';
...
CFG.UseParfor = env_true('ADAPTIVE_REQ_TEST38_USE_PARFOR', false);
CFG.UseParallelTraining = env_true('ADAPTIVE_REQ_TEST38_USE_PARALLEL_TRAINING', false);
CFG.ParallelPoolType = "threads";
CFG.TrainFraction = 0.70;
CFG.MaxPatchesPerCondition = env_number('ADAPTIVE_REQ_TEST38_MAX_PATCHES_PER_CONDITION', 300);
CFG.MaxRowsForModelTraining = env_number('ADAPTIVE_REQ_TEST38_MAX_MODEL_TRAIN_ROWS', 250000);
CFG.MaxRowsForModelEvaluation = env_number('ADAPTIVE_REQ_TEST38_MAX_MODEL_EVAL_ROWS', 250000);
if CFG.QuickMode
    CFG.MaxPatchesPerCondition = 550;
    CFG.MaxRowsForModelTraining = env_number('ADAPTIVE_REQ_TEST38_MAX_MODEL_TRAIN_ROWS', 0);
    CFG.MaxRowsForModelEvaluation = env_number('ADAPTIVE_REQ_TEST38_MAX_MODEL_EVAL_ROWS', 0);
end
CFG.DiagnosticMapInterpScale = env_number('ADAPTIVE_REQ_TEST38_MAP_INTERP_SCALE', 4);
...
elseif CFG.MediumMode
    CFG.Frequencies = [350 550];
    CFG.M = 2;
    CFG.RegimeSet = "full_regimes_medium_freq_M2";
    CFG.TreeLearners = 140;
else
    CFG.Frequencies = [350 450 550 650];
    CFG.M = [1.5 2 2.5 3 3.5 4];
    CFG.DxList = env_number_list('ADAPTIVE_REQ_TEST38_DX_LIST_MM', ...
        [0.15 0.2 0.25 0.3]) * 1e-3;
    CFG.dx = CFG.DxList(1);
    CFG.dz = CFG.dx;
    CFG.MaxPatchesPerCondition = env_number( ...
        'ADAPTIVE_REQ_TEST38_MAX_PATCHES_PER_CONDITION', 0);
    CFG.MaxRowsForModelTraining = env_number( ...
        'ADAPTIVE_REQ_TEST38_MAX_MODEL_TRAIN_ROWS', 500000);
    CFG.MaxRowsForModelEvaluation = env_number( ...
        'ADAPTIVE_REQ_TEST38_MAX_MODEL_EVAL_ROWS', 500000);
    CFG.RegimeSet = "full";
end
CFG.Frequencies = env_number_list('ADAPTIVE_REQ_TEST38_FREQUENCIES_HZ', CFG.Frequencies);
CFG.M = env_number_list('ADAPTIVE_REQ_TEST38_M_LIST', CFG.M);
CFG.OutputName = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST38_OUTPUT_NAME'))));
if CFG.OutputName == ""
    CFG.OutputName = "test_38_velocity_field_diverse_q_training";
end
end
...
T_all = vertcat(parts{:});
T_all.dataset = repmat("test38_synthetic_v2", height(T_all), 1);
...
export_fig(fig, fullfile(OUT.figure_dir,'test38_summary_bars.png'));
end
...
title('Held-out error vs predicted composition','FontWeight','normal', ...
    'Interpreter','none');
grid on; colorbar;
export_fig(fig, fullfile(OUT.figure_dir,'test38_error_vs_predicted_purity.png'));
end
...
grid(ax,'on');
export_fig(fig, fullfile(OUT.figure_dir,'test38_composition_diagnostics.png'));
end
...
title('Test 38 held-out model ranking','Interpreter','none','FontWeight','normal');
export_fig(fig, fullfile(OUT.figure_dir,'test38_model_ranking.png'));
end

function plot_training_maps(T, OUT, CFG)
preferred = ["q_spectrum_only","delta_logk_theory_composition", ...
    "q_spectrum_plus_theory_composition","theory_discrete"];
...
for ki = 1:numel(keys)
    ...
    out_dir = fullfile(OUT.map_dir, sanitize(X0.case_id(1)), ...
        ...
        "M" + string(X0.M(1)));
    if exist(out_dir,'dir') ~= 7, mkdir(out_dir); end
    out_file = fullfile(out_dir, "test38_map__" + sanitize(keys(ki)) + ".png");
    export_fig(fig, out_file);
end
end
