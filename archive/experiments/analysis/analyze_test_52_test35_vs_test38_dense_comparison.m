%% analyze_test_52_test35_vs_test38_dense_comparison.m
% Test 52: dense same-map comparison of frozen Test 35 and Test 38 models.
%
% This script recalculates REQ on representative maps only. It never trains
% or modifies either saved model bundle. The purpose is to answer a narrow
% diagnostic question: were the excellent Test 35 results due to the model
% itself, or to the sparse/patch-level validation setting?  Test 35 and Test
% 38 are applied to exactly the same dense REQ feature tables here.
...
write_config_json(CFG, fullfile(OUT.root_dir,'test52_configuration.json'));

fprintf('\nTest 52: frozen Test 35 vs Test 38 on identical dense REQ maps\n');
fprintf('Mode: %s | target REQ step %.3f mm | save all maps: %d\n', ...
    CFG.Mode, 1e3*CFG.TargetStepM, CFG.SaveAllMaps);
fprintf('No training. Frozen Test35 source: %s\n', CFG.Test35BundleFile);
fprintf('No training. Frozen Test38 source: %s\n', CFG.Test38BundleFile);
...
BUNDLES = load_model_bundles(CFG);
ALL_BASE_FEATURES = unique([BUNDLES(1).base_features; BUNDLES(2).base_features], 'stable');
assert_no_forbidden_predictors(BUNDLES(1).base_features, "Test35");
assert_no_forbidden_predictors(BUNDLES(2).base_features, "Test38");
...
for ci = 1:numel(CONDITIONS)
    ...
    for mi = 1:numel(CFG.M)
        ...
        timer = tic;
        [T_pred, T_base] = evaluate_dense_condition(CND, M, key, BUNDLES, ...
            ALL_BASE_FEATURES, CFG, OUT);
...
T33 = struct();
if CFG.CompareTest33
    T33 = load_and_summarize_test33_reference(root_dir, CFG, OUT);
end

writetable(T_all, fullfile(OUT.table_dir,'test52_dense_patch_level_results.csv'));
writetable(T_roi, fullfile(OUT.table_dir,'test52_roi_summary.csv'));
writetable(T_overall, fullfile(OUT.table_dir,'test52_summary_overall.csv'));
writetable(T_by_case, fullfile(OUT.table_dir,'test52_summary_by_case.csv'));
writetable(T_by_regime, fullfile(OUT.table_dir,'test52_summary_by_regime.csv'));
writetable(T_by_frequency, fullfile(OUT.table_dir,'test52_summary_by_frequency.csv'));
writetable(T_by_M, fullfile(OUT.table_dir,'test52_summary_by_M.csv'));
writetable(T_by_roi, fullfile(OUT.table_dir,'test52_summary_by_roi.csv'));

safe_plot(@() plot_summary_figures(T_overall, T_by_case, T_by_frequency, T_by_M, T_by_roi, OUT), ...
    'summary figures');
if CFG.CompareTest33 && isfield(T33,'overall') && ~isempty(T33.overall)
    safe_plot(@() plot_test33_reference_comparison(T_overall, T33.overall, T33.by_roi, OUT), ...
        'Test33 reference comparison');
end

save(fullfile(OUT.data_dir,'test52_compact_results.mat'), ...
    'CFG','T_overall','T_by_case','T_by_regime','T_by_frequency','T_by_M', ...
    'T_by_roi','T_roi','T33','BUNDLES','-v7.3');
...
mode = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST52_MODE'))));
if mode == "", mode = "quick"; end
assert(ismember(mode, ["quick","full"]), 'ADAPTIVE_REQ_TEST52_MODE must be quick or full.');
CFG = struct();
CFG.Mode = mode;
CFG.QuickMode = mode == "quick";
CFG.ValidateOnly = env_true('ADAPTIVE_REQ_TEST52_VALIDATE_ONLY', false);
CFG.SaveAllMaps = env_true('ADAPTIVE_REQ_TEST52_SAVE_ALL_MAPS', true);
CFG.UseParfor = env_true('ADAPTIVE_REQ_TEST52_USE_PARFOR', false);
CFG.TargetStepM = env_num('ADAPTIVE_REQ_TEST52_TARGET_STEP_M', 0.5e-3);
CFG.CompareTest33 = env_true('ADAPTIVE_REQ_TEST52_COMPARE_TEST33', false);
CFG.Test33Source = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST52_TEST33_SOURCE'))));
if CFG.Test33Source == ""
    if CFG.QuickMode || CFG.ValidateOnly
        CFG.Test33Source = "quick";
    else
        CFG.Test33Source = "full";
    end
end
CFG.Test35ModelSource = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST52_TEST35_MODEL_SOURCE'))));
if CFG.Test35ModelSource == ""
    CFG.Test35ModelSource = "full";
end
CFG.Test38ModelSource = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST52_TEST38_MODEL_SOURCE'))));
if CFG.Test38ModelSource == ""
    if CFG.QuickMode || CFG.ValidateOnly
        CFG.Test38ModelSource = "medium";
    else
        CFG.Test38ModelSource = "full";
    end
end
CFG.Test35BundleFile = resolve_test35_bundle(root_dir, CFG.Test35ModelSource);
CFG.Test38BundleFile = resolve_test38_bundle(root_dir, CFG.Test38ModelSource);
CFG.dx = 0.2e-3;
CFG.dz = 0.2e-3;
CFG.Lx = 0.05;
CFG.Lz = 0.05;
CFG.cs_guess = 3;
CFG.M = [2 3];
CFG.RandomSeed = 52001;
CFG.DistanceEdgesMm = [0 0.5 1 2 4 8 Inf];
CFG.CenterRoiSizeMm = 8;
CFG.SoftCoreMinDistanceMm = 8;
CFG.HardCoreMinDistanceMm = 4;
CFG.ModelsToPlot = ["T35_q_spectrum_only","T38_q_spectrum_only", ...
    "T35_q_spectrum_plus_composition","T38_q_spectrum_plus_composition", ...
    "T35_q_spectrum_plus_theory_composition","T38_q_spectrum_plus_theory_composition"];
CFG.ModelsToEvaluate = ["q_spectrum_only", ...
    "q_spectrum_plus_composition","q_spectrum_plus_theory_composition"];
CFG.Test33Strategies = ["local_baseline","mixedness_logk_corrected", ...
    "mixedness_q_candidate_selector"];
CFG.REQ.Profile = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST52_REQ_PROFILE'))));
...
switch CFG.REQ.Profile
    ...
    case {"dense_default","old_test52"}
        CFG.REQ.Nbins_auto_oversample = 2;
        CFG.REQ.Nbins_min = 128;
    otherwise
        error('Unknown ADAPTIVE_REQ_TEST52_REQ_PROFILE: %s', CFG.REQ.Profile);
end
CFG.REQ.Nbins_auto_oversample = env_num('ADAPTIVE_REQ_TEST52_NBINS_OVERSAMPLE', CFG.REQ.Nbins_auto_oversample);
CFG.REQ.Nbins_min = env_num('ADAPTIVE_REQ_TEST52_NBINS_MIN', CFG.REQ.Nbins_min);
...
if CFG.QuickMode
    OUT.root_dir = fullfile(root_dir,'outputs','test_52_test35_vs_test38_dense_comparison','quick');
else
    OUT.root_dir = fullfile(root_dir,'outputs','test_52_test35_vs_test38_dense_comparison');
end
if CFG.ValidateOnly
...
function file = resolve_test35_bundle(root_dir, source)
...
switch lower(source)
    case "quick"
        file = fullfile(root_dir, 'outputs', ...
            'test_35_spectral_composition_to_q_model', 'quick', 'models', ...
            'test35_spectral_composition_q_models.mat');
    otherwise
        file = fullfile(root_dir, 'outputs', ...
            'test_35_spectral_composition_to_q_model', 'models', ...
            'test35_spectral_composition_q_models.mat');
end
end

function file = resolve_test38_bundle(root_dir, source)
source = string(source);
if isfile(source)
    file = char(source);
    return;
...
function BUNDLES = load_model_bundles(CFG)
BUNDLES = repmat(struct('label',"",'source',"",'file',"",'MODELS',[], ...
    'base_features',string.empty(0,1)), 2, 1);
BUNDLES(1) = load_one_bundle("T35", CFG.Test35ModelSource, CFG.Test35BundleFile);
BUNDLES(2) = load_one_bundle("T38", CFG.Test38ModelSource, CFG.Test38BundleFile);
end

function B = load_one_bundle(label, source, file)
assert(exist(file,'file') == 2, 'Missing %s model bundle: %s', label, file);
fprintf('Loading frozen %s bundle (%s): %s\n', label, source, file);
S = load(file, 'MODEL_BUNDLE');
MB = S.MODEL_BUNDLE;
B = struct('label',string(label),'source',string(source),'file',string(file), ...
    'MODELS',MB.MODELS,'base_features',string(MB.BASE_FEATURES(:)));
fprintf('Loaded %s bundle with %d model entries and %d primitive predictors.\n', ...
    label, numel(B.MODELS.model_names), numel(B.base_features));
end

function assert_no_forbidden_predictors(features, label)
...
for p = bad_patterns
    ...
    assert(isempty(hit), 'Forbidden %s predictor detected: %s', label, strjoin(hit, ', '));
end
fprintf('%s base predictors passed leakage guard (%d predictors).\n', label, numel(features));
end
...
function [T_pred, F] = evaluate_dense_condition(CND, M, key, BUNDLES, ALL_BASE_FEATURES, CFG, OUT)
...
F = ensure_predictor_columns(F, ALL_BASE_FEATURES, CFG);
parts = cell(numel(BUNDLES),1);
for bi = 1:numel(BUNDLES)
    parts{bi} = apply_models(F, BUNDLES(bi).MODELS, BUNDLES(bi).base_features, ...
        "test52_dense_validation", CFG.ModelsToEvaluate, BUNDLES(bi).label);
end
...
n = height(F);
F.dataset = repmat("test52_dense_validation", n, 1);
...
function T_out = apply_models(T, MODELS, base_features, dataset_name, keep_models, bundle_label)
...
for mi = 1:numel(model_names)
    ...
    R.model_name = repmat(bundle_label + "_" + name, height(R), 1);
    R.model_variant = repmat(name, height(R), 1);
    R.model_bundle = repmat(bundle_label, height(R), 1);
...
if exist(source,'file') ~= 2
    warning('Test52:MissingTest33', 'Missing Test33 reference table: %s', source);
    return;
end
...
if isempty(Tlong)
    warning('Test52:NoTest33Strategies', 'No requested Test33 strategies were found.');
    return;
end
...
T33.notes = table("test33_reference", string(source), height(Tlong), ...
    "These summaries use cached Test 33 test predictions. They are not regenerated on the dense Test 52 fields because Test 33 depends on Test 31/32 intermediate operational maps.", ...
    'VariableNames', {'source_name','source_file','N_long_rows','note'});
writetable(T33.overall, fullfile(OUT.table_dir,'test52_test33_reference_summary_overall.csv'));
writetable(T33.by_geometry, fullfile(OUT.table_dir,'test52_test33_reference_summary_by_geometry.csv'));
writetable(T33.by_frequency, fullfile(OUT.table_dir,'test52_test33_reference_summary_by_frequency.csv'));
writetable(T33.by_M, fullfile(OUT.table_dir,'test52_test33_reference_summary_by_M.csv'));
writetable(T33.by_roi, fullfile(OUT.table_dir,'test52_test33_reference_summary_by_roi.csv'));
writetable(T33.by_distance, fullfile(OUT.table_dir,'test52_test33_reference_summary_by_distance.csv'));
writetable(T33.by_side, fullfile(OUT.table_dir,'test52_test33_reference_summary_by_side.csv'));
writetable(T33.notes, fullfile(OUT.table_dir,'test52_test33_reference_notes.csv'));
...
A.source_group = repmat("Test52 dense, matched REQ", height(A), 1);
B = T33_overall;
...
export_fig(fig, fullfile(OUT.figure_dir,'test52_test33_reference_comparison.png'));
end
...
T35 = T(T.model_name=="T35_q_spectrum_plus_composition",:);
T38 = T(T.model_name=="T38_q_spectrum_plus_composition",:);
T35q = T(T.model_name=="T35_q_spectrum_only",:);
T38q = T(T.model_name=="T38_q_spectrum_only",:);
T35tc = T(T.model_name=="T35_q_spectrum_plus_theory_composition",:);
T38tc = T(T.model_name=="T38_q_spectrum_plus_theory_composition",:);
...
plot_map(nexttile(tl), rows_to_grid(T35, T35.predicted_patch_purity, nz, nx), 'Test35 predicted purity');
plot_map(nexttile(tl), rows_to_grid(T38, T38.predicted_patch_purity, nz, nx), 'Test38 predicted purity');
plot_map(nexttile(tl), rows_to_grid(T35q, T35q.sws_pred, nz, nx), 'Test35 q-spectrum SWS');
plot_map(nexttile(tl), rows_to_grid(T38q, T38q.sws_pred, nz, nx), 'Test38 q-spectrum SWS');
plot_map(nexttile(tl), rows_to_grid(T35, T35.sws_pred, nz, nx), 'Test35 q + composition SWS');
plot_map(nexttile(tl), rows_to_grid(T38, T38.sws_pred, nz, nx), 'Test38 q + composition SWS');
plot_map(nexttile(tl), rows_to_grid(T35tc, T35tc.sws_pred, nz, nx), 'Test35 q + theory + composition SWS');
plot_map(nexttile(tl), rows_to_grid(T38tc, T38tc.sws_pred, nz, nx), 'Test38 q + theory + composition SWS');
plot_map(nexttile(tl), rows_to_grid(T35q, T35q.sws_abs_error_pct, nz, nx), 'Test35 q-spectrum abs error (%)');
plot_map(nexttile(tl), rows_to_grid(T38q, T38q.sws_abs_error_pct, nz, nx), 'Test38 q-spectrum abs error (%)');
plot_map(nexttile(tl), rows_to_grid(T35, T35.sws_abs_error_pct, nz, nx), 'Test35 q+comp abs error (%)');
plot_map(nexttile(tl), rows_to_grid(T38, T38.sws_abs_error_pct, nz, nx), 'Test38 q+comp abs error (%)');
map35 = rows_to_grid(T35, T35.sws_pred, nz, nx);
map38 = rows_to_grid(T38, T38.sws_pred, nz, nx);
plot_map(nexttile(tl), map38 - map35, 'Test38 minus Test35 q+comp SWS (m/s)');
...
export_fig(fig, fullfile(out_dir, "test52_map__" + sanitize(key) + ".png"));
end

function plot_roi_overlay_pair(T, key, OUT, CFG)
base = T(T.model_name == "T38_q_spectrum_plus_composition",:);
if isempty(base), base = T(T.model_name == T.model_name(1),:); end
[true_map,nz,nx] = rows_to_grid(base, base.true_SWS);
...
if exist(out_dir,'dir') ~= 7, mkdir(out_dir); end
export_fig(fig, fullfile(out_dir, "test52_roi_overlay__" + sanitize(key) + ".png"));
end
...
legend(ax,'Location','best','Interpreter','none');
ax = nexttile(tl);
P = T_roi(T_roi.model_name=="T38_q_spectrum_plus_composition",:);
...
export_fig(fig, fullfile(OUT.figure_dir,'test52_summary_diagnostics.png'));
end
...
s(x=="T35_q_spectrum_only") = "Test35 q spectrum only";
s(x=="T35_q_spectrum_plus_composition") = "Test35 q spectrum + composition";
s(x=="T35_q_spectrum_plus_theory_composition") = "Test35 q spectrum + theory + composition";
s(x=="T38_q_spectrum_only") = "Test38 q spectrum only";
s(x=="T38_q_spectrum_plus_composition") = "Test38 q spectrum + composition";
s(x=="T38_q_spectrum_plus_theory_composition") = "Test38 q spectrum + theory + composition";
...
try
    ...
    warning('Test52:PlotFailed', 'Plot failed (%s): %s', label, ME.message);
end
end
...
fprintf('\n================ Test 52 dense summary ================\n');
disp(T_overall(:, {'model_name','N','MAPE_pct','mean_signed_error_pct','high_error20_pct'}));
P = T_overall(T_overall.model_name=="T38_q_spectrum_plus_composition",:);
if ~isempty(P)
    fprintf('Test38 q+composition dense representative MAPE %.2f%%, high>20 %.2f%%.\n', ...
        P.MAPE_pct, P.high_error20_pct);
end
X = T_case(T_case.model_name=="T38_q_spectrum_plus_composition",:);
if ~isempty(X)
    X = sortrows(X,'MAPE_pct','descend');
    fprintf('Worst dense Test38 q+composition case: %s (%s), MAPE %.2f%%.\n', ...
        X.case_id(1), X.seen_status(1), X.MAPE_pct(1));
end
R = T_roi(T_roi.model_name=="T38_q_spectrum_plus_composition",:);
if ~isempty(R)
