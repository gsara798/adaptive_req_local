%% analyze_test_63_controlled_dense_frequency_regime_matrix.m
% Test 63: controlled dense frequency/regime matrix for the frozen Test 61 gate.
%
% This script is the controlled counterpart to Test 62. It recalculates dense
% REQ maps on the same geometries, frequencies, and field regimes so M,
% frequency, geometry, and regime effects can be compared without the OOD
% condition mixing used in representative-map tests. No base q model and no
% correction gate is retrained here.
...
write_config_json(CFG, fullfile(OUT.root_dir,'test63_configuration.json'));

fprintf('\nTest 63: controlled dense frequency/regime matrix for frozen Test 61 gate\n');
...
fprintf('Controlled grid: geometries x frequencies x regimes, with at least one aligned source.\n');
...
for ci = 1:numel(CONDITIONS)
    ...
    alignment_parts{end+1,1} = table(string(CND.case.case_id), string(CND.case.case_family), ...
        string(CND.regime.regime_id), CND.f0, ...
        sum(abs(T_long.M-2)<1e-8), sum(abs(T_long.M-3)<1e-8), height(W), ...
        100*height(W)/max(sum(abs(T_long.M-2)<1e-8),1), ...
        100*height(W)/max(sum(abs(T_long.M-3)<1e-8),1), ...
        'VariableNames', {'case_id','case_family','field_regime_ood','f0', ...
        'N_M2_dense','N_M3_dense','N_aligned','aligned_over_M2_pct','aligned_over_M3_pct'}); %#ok<AGROW>
    ...
    if CFG.SaveAllMaps && should_save_map(ci, CFG)
        plot_test63_maps(T_strat, T_long, CND, OUT, CFG);
...
T_alignment = vertcat(alignment_parts{:});
...
T_by_case_frequency_regime = summarize_strategy_predictions(T_all, ...
    ["strategy_name","case_id","case_family","f0","field_regime_ood"]);
T_by_case_frequency = summarize_strategy_predictions(T_all, ...
    ["strategy_name","case_id","case_family","f0"]);
T_by_case_regime = summarize_strategy_predictions(T_all, ...
    ["strategy_name","case_id","case_family","field_regime_ood"]);
T_by_roi = summarize_strategy_predictions(T_all(T_all.roi_region ~= "other",:), ...
    ["strategy_name","case_id","roi_region"]);
T_by_roi_frequency = summarize_strategy_predictions(T_all(T_all.roi_region ~= "other",:), ...
    ["strategy_name","case_id","roi_region","f0"]);
T_by_roi_regime = summarize_strategy_predictions(T_all(T_all.roi_region ~= "other",:), ...
    ["strategy_name","case_id","roi_region","field_regime_ood"]);
T_harm = summarize_harm(T_all);

writetable(T_all, fullfile(OUT.table_dir,'test63_dense_strategy_patch_level_results.csv'));
writetable(T_base_long, fullfile(OUT.table_dir,'test63_dense_base_M2_M3_long_results.csv'));
writetable(T_alignment, fullfile(OUT.table_dir,'test63_alignment_coverage_by_condition.csv'));
writetable(T_overall, fullfile(OUT.table_dir,'test63_strategy_summary_overall.csv'));
writetable(T_by_case, fullfile(OUT.table_dir,'test63_strategy_summary_by_case.csv'));
writetable(T_by_regime, fullfile(OUT.table_dir,'test63_strategy_summary_by_regime.csv'));
writetable(T_by_frequency, fullfile(OUT.table_dir,'test63_strategy_summary_by_frequency.csv'));
writetable(T_by_case_frequency_regime, fullfile(OUT.table_dir,'test63_strategy_summary_by_case_frequency_regime.csv'));
writetable(T_by_case_frequency, fullfile(OUT.table_dir,'test63_strategy_summary_by_case_frequency.csv'));
writetable(T_by_case_regime, fullfile(OUT.table_dir,'test63_strategy_summary_by_case_regime.csv'));
writetable(T_by_roi, fullfile(OUT.table_dir,'test63_strategy_summary_by_roi.csv'));
writetable(T_by_roi_frequency, fullfile(OUT.table_dir,'test63_strategy_summary_by_roi_frequency.csv'));
writetable(T_by_roi_regime, fullfile(OUT.table_dir,'test63_strategy_summary_by_roi_regime.csv'));
writetable(T_harm, fullfile(OUT.table_dir,'test63_correction_harm_summary.csv'));

safe_plot(@() plot_test63_summary_figures(T_overall, T_by_case, T_by_frequency, T_by_roi, T_harm, OUT), ...
    'summary figures');
safe_plot(@() plot_controlled_matrix_figures(T_by_case_frequency, T_by_case_regime, ...
    T_by_roi_frequency, T_by_roi_regime, OUT), 'controlled matrix figures');

save(fullfile(OUT.data_dir,'test63_compact_results.mat'), ...
    'CFG','T_overall','T_by_case','T_by_regime','T_by_frequency', ...
    'T_by_case_frequency_regime','T_by_case_frequency','T_by_case_regime', ...
    'T_by_roi','T_by_roi_frequency','T_by_roi_regime','T_harm','T_alignment','-v7.3');

print_test63_summary(T_overall, T_by_roi, T_harm, OUT);
...
mode = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST63_MODE'))));
if mode == "", mode = "quick"; end
assert(ismember(mode, ["quick","full"]), 'ADAPTIVE_REQ_TEST63_MODE must be quick or full.');
CFG = struct();
CFG.Mode = mode;
CFG.QuickMode = mode == "quick";
CFG.ValidateOnly = env_true('ADAPTIVE_REQ_TEST63_VALIDATE_ONLY', false);
CFG.SaveAllMaps = env_true('ADAPTIVE_REQ_TEST63_SAVE_ALL_MAPS', true);
CFG.UseParfor = env_true('ADAPTIVE_REQ_TEST63_USE_PARFOR', false);
CFG.TargetStepM = env_num('ADAPTIVE_REQ_TEST63_TARGET_STEP_M', 0.5e-3);
CFG.MaxMapConditions = env_num('ADAPTIVE_REQ_TEST63_MAX_MAP_CONDITIONS', 0);
CFG.ModelSource = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST63_MODEL_SOURCE'))));
if CFG.ModelSource == ""
...
CFG.Test61Source = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST63_TEST61_SOURCE'))));
if CFG.Test61Source == ""
...
CFG.REQ.Profile = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST63_REQ_PROFILE'))));
...
switch CFG.REQ.Profile
    ...
    otherwise
        error('Unknown ADAPTIVE_REQ_TEST63_REQ_PROFILE: %s', CFG.REQ.Profile);
end
CFG.REQ.Nbins_auto_oversample = env_num('ADAPTIVE_REQ_TEST63_NBINS_OVERSAMPLE', CFG.REQ.Nbins_auto_oversample);
CFG.REQ.Nbins_min = env_num('ADAPTIVE_REQ_TEST63_NBINS_MIN', CFG.REQ.Nbins_min);
...
if CFG.QuickMode
    OUT.root_dir = fullfile(root_dir,'outputs','test_63_controlled_dense_frequency_regime_matrix','quick');
else
    OUT.root_dir = fullfile(root_dir,'outputs','test_63_controlled_dense_frequency_regime_matrix');
end
if CFG.ValidateOnly
...
cases = [
    ...
    homogeneous_case("homogeneous_cs4", 4, "seen")
    bilayer_case("bilayer_2_4", 2, 4, 0, "seen")
    inclusion_case("circular_inclusion_2_4", 2, 4, [0.025 0.025], 0.008, "seen")
    three_material_case("three_material_2_3_4", 2, 3, 4, "seen")
    ];
regimes = [
    regime("directional_2D_angle0","directional_2D",true,1,true,"ranges","fibonacci",[0 0],[0 2*pi],[0 pi],11)
    regime("directional_2D_angle30","directional_2D",true,1,true,"ranges","fibonacci",[deg2rad(30) deg2rad(30)],[0 2*pi],[0 pi],13)
    regime("diffuse_2D_seed1","diffuse_2D",true,128,true,"ranges","random",[0 2*pi],[0 2*pi],[0 pi],17)
    regime("partial_3D_12src","partial_3D",false,12,true,"ranges","fibonacci",[0 2*pi],[0 2*pi],[0 pi],19)
    regime("diffuse_3D_seed1","diffuse_3D",false,128,true,"ranges","random",[0 2*pi],[0 2*pi],[0 pi],23)
    ];
if CFG.QuickMode
    cases = cases([1 3 4 5 6]);
    freqs = [200 400];
    regimes = regimes([1 3 5]);
else
...
T_pred = apply_models(F, MODELS, BASE_FEATURES, "test63_dense_test61_validation", CFG.ModelsToEvaluate);
end
...
n = height(F);
F.dataset = repmat("test63_dense_test61_validation", n, 1);
...
if exist(out_dir,'dir') ~= 7, mkdir(out_dir); end
export_fig(fig, fullfile(out_dir, "test63_roi_overlay__" + sanitize(key) + ".png"));
end
...
function plot_test63_summary_figures(T_overall, T_case, T_freq, T_roi, T_harm, OUT)
...
export_fig(fig, fullfile(OUT.figure_dir,'test63_summary_diagnostics.png'));
end

function plot_controlled_matrix_figures(T_case_freq, T_case_regime, T_roi_freq, T_roi_regime, OUT)
main_strategies = ["fixed_M2","fixed_M3","interface_penalized_selector", ...
    "learned_benefit_harm_gate","conservative_benefit_harm_gate"];
T_case_freq = T_case_freq(ismember(T_case_freq.strategy_name, main_strategies),:);
T_case_regime = T_case_regime(ismember(T_case_regime.strategy_name, main_strategies),:);

fig = figure('Color','w','Units','centimeters','Position',[1 1 38 22]);
cases = unique(string(T_case_freq.case_id), 'stable');
nrow = ceil(numel(cases)/2);
tl = tiledlayout(fig,nrow,2,'TileSpacing','compact','Padding','compact');
for ci = 1:numel(cases)
    ax = nexttile(tl); hold(ax,'on');
    for s = main_strategies
        X = T_case_freq(string(T_case_freq.case_id)==cases(ci) & T_case_freq.strategy_name==s,:);
        if isempty(X), continue; end
        X = aggregate_strategy_for_plot(X, ["strategy_name","f0"]);
        X = sortrows(X,'f0');
        plot(ax, X.f0, X.MAPE_pct, '-o', 'DisplayName', pretty_strategy(s));
    end
    title(ax, strrep(cases(ci),"_"," "), 'FontWeight','normal');
    xlabel(ax,'Frequency (Hz)'); ylabel(ax,'MAPE (%)'); grid(ax,'on');
    if ci == 1, legend(ax,'Location','bestoutside','Interpreter','none'); end
end
title(tl,'Controlled matrix: MAPE versus frequency by geometry');
export_fig(fig, fullfile(OUT.figure_dir,'test63_controlled_mape_vs_frequency_by_geometry.png'));

fig = figure('Color','w','Units','centimeters','Position',[1 1 38 22]);
tl = tiledlayout(fig,nrow,2,'TileSpacing','compact','Padding','compact');
for ci = 1:numel(cases)
    ax = nexttile(tl); hold(ax,'on');
    X = T_case_regime(string(T_case_regime.case_id)==cases(ci),:);
    if isempty(X), continue; end
    X = aggregate_strategy_for_plot(X, ["strategy_name","field_regime_ood"]);
    labels = pretty_strategy(X.strategy_name) + " | " + pretty_regime(X.field_regime_ood);
    [~,ord] = sort(X.MAPE_pct,'descend');
    barh(ax, categorical(labels(ord)), X.MAPE_pct(ord));
    title(ax, strrep(cases(ci),"_"," "), 'FontWeight','normal');
    xlabel(ax,'MAPE (%)'); grid(ax,'on');
end
title(tl,'Controlled matrix: MAPE by field regime and geometry');
export_fig(fig, fullfile(OUT.figure_dir,'test63_controlled_mape_by_regime_geometry.png'));

roi_keep = ["homogeneous_center_8mm","soft_core_gt8mm","hard_core_gt4mm", ...
    "interface_0_0p5mm","interface_0p5_1mm","interface_1_2mm"];
R = T_roi_freq(ismember(T_roi_freq.strategy_name, main_strategies) & ...
    ismember(T_roi_freq.roi_region, roi_keep),:);
if ~isempty(R)
    fig = figure('Color','w','Units','centimeters','Position',[1 1 38 22]);
    roi_cases = unique(string(R.case_id), 'stable');
    nrow = ceil(numel(roi_cases)/2);
    tl = tiledlayout(fig,nrow,2,'TileSpacing','compact','Padding','compact');
    for ci = 1:numel(roi_cases)
        ax = nexttile(tl); hold(ax,'on');
        Xc = R(string(R.case_id)==roi_cases(ci) & ...
            ismember(R.roi_region, ["hard_core_gt4mm","soft_core_gt8mm","homogeneous_center_8mm"]),:);
        Xc = Xc(ismember(Xc.strategy_name, ["fixed_M2","fixed_M3","learned_benefit_harm_gate","conservative_benefit_harm_gate"]),:);
        for s = unique(Xc.strategy_name,'stable')'
            for roi = unique(Xc.roi_region,'stable')'
                X = Xc(Xc.strategy_name==s & Xc.roi_region==roi,:);
                if isempty(X), continue; end
                X = aggregate_strategy_for_plot(X, ["strategy_name","roi_region","f0"]);
                X = sortrows(X,'f0');
                plot(ax, X.f0, X.MAPE_pct, '-o', ...
                    'DisplayName', pretty_strategy(s) + " | " + pretty_region(roi));
            end
        end
        title(ax, strrep(roi_cases(ci),"_"," "), 'FontWeight','normal');
        xlabel(ax,'Frequency (Hz)'); ylabel(ax,'ROI MAPE (%)'); grid(ax,'on');
        if ci == 1, legend(ax,'Location','bestoutside','Interpreter','none'); end
    end
    title(tl,'Controlled matrix: core ROI MAPE versus frequency');
    export_fig(fig, fullfile(OUT.figure_dir,'test63_controlled_roi_mape_vs_frequency.png'));
end

Rr = T_roi_regime(ismember(T_roi_regime.strategy_name, main_strategies) & ...
    ismember(T_roi_regime.roi_region, roi_keep),:);
if ~isempty(Rr)
    Rr = aggregate_strategy_for_plot(Rr, ["strategy_name","roi_region","field_regime_ood"]);
    fig = figure('Color','w','Units','centimeters','Position',[1 1 32 18]);
    labels = pretty_strategy(Rr.strategy_name) + " | " + pretty_region(Rr.roi_region) + ...
        " | " + pretty_regime(Rr.field_regime_ood);
    [~,ord] = sort(Rr.MAPE_pct,'descend');
    ord = ord(1:min(numel(ord),36));
    barh(categorical(labels(ord)), Rr.MAPE_pct(ord));
    xlabel('MAPE (%)'); title('Worst ROI/regime combinations in controlled matrix','FontWeight','normal');
    grid on;
    export_fig(fig, fullfile(OUT.figure_dir,'test63_controlled_worst_roi_regime_combinations.png'));
end
end

function plot_test63_maps(T, Tlong, CND, OUT, CFG)
...
export_fig(fig, fullfile(out_dir, "test63_map__" + sanitize(key) + ".png"));
end
...
s = string(x);
s = strrep(s, "directional_2D_angle0", "Directional 2D 0 deg");
s = strrep(s, "directional_2D_angle30", "Directional 2D 30 deg");
s = strrep(s, "diffuse_2D_seed1", "Diffuse 2D");
s = strrep(s, "partial_3D_12src", "Partial 3D, 12 src");
s = strrep(s, "diffuse_3D_seed1", "Diffuse 3D");
s = strrep(s, "_", " ");
end
...
function print_test63_summary(T_overall, T_roi, T_harm, OUT)
fprintf('\n================ Test 63 dense Test61 summary ================\n');
