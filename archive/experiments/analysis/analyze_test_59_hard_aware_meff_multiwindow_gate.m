%% analyze_test_59_hard_aware_meff_multiwindow_gate.m
% Test 59: hard-aware M-effective + multi-window gate.
%
...
write_config_json(CFG, fullfile(OUT.root_dir,'test59_configuration.json'));

fprintf('\nTest 59: hard-aware M-eff + multi-window gate\n');
...
assert(any(abs(Mvals-2)<1e-8), 'Test59 requires M=2 rows.');
assert(any(abs(Mvals-3)<1e-8), 'Test59 requires M=3 rows. Rerun Test55 with ADAPTIVE_REQ_TEST55_M_LIST including 3.');
...
writetable(remove_cell_columns(T_strategies), fullfile(OUT.table_dir,'test59_patch_level_results.csv'));
writetable(T_overall, fullfile(OUT.table_dir,'test59_strategy_summary_overall.csv'));
writetable(T_by_case, fullfile(OUT.table_dir,'test59_strategy_summary_by_case.csv'));
writetable(T_by_freq, fullfile(OUT.table_dir,'test59_strategy_summary_by_frequency.csv'));
writetable(T_by_regime, fullfile(OUT.table_dir,'test59_strategy_summary_by_regime.csv'));
writetable(T_by_roi, fullfile(OUT.table_dir,'test59_strategy_summary_by_roi.csv'));
writetable(T_by_case_roi, fullfile(OUT.table_dir,'test59_strategy_summary_by_case_roi.csv'));
writetable(T_harm, fullfile(OUT.table_dir,'test59_correction_harm_summary.csv'));
writetable(T_success, fullfile(OUT.table_dir,'test59_success_criteria_summary.csv'));
...
save(fullfile(OUT.data_dir,'test59_hard_aware_gate_results.mat'), ...
...
mode = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST59_MODE'))));
if mode == "", mode = "quick"; end
assert(ismember(mode, ["quick","full"]), 'ADAPTIVE_REQ_TEST59_MODE must be quick or full.');
CFG = struct();
CFG.Mode = mode;
CFG.QuickMode = mode == "quick";
CFG.ValidateOnly = env_true('ADAPTIVE_REQ_TEST59_VALIDATE_ONLY', false);
CFG.SaveAllMaps = env_true('ADAPTIVE_REQ_TEST59_SAVE_ALL_MAPS', true);
CFG.RandomSeed = 59001;
CFG.SourceCsv = env_string('ADAPTIVE_REQ_TEST59_SOURCE_CSV', ...
    fullfile(root_dir,'outputs','test_55_controlled_field_frequency_roi_sweep', ...
    'tables','test55_patch_level_results.csv'));
CFG.BaseModel = env_string('ADAPTIVE_REQ_TEST59_BASE_MODEL', ...
    "q_spectrum_plus_composition");
CFG.TrainFraction = env_number('ADAPTIVE_REQ_TEST59_TRAIN_FRACTION', 0.70);
CFG.CsGuess = env_number('ADAPTIVE_REQ_TEST59_CS_GUESS', 3.0);
CFG.TreeLearners = env_number('ADAPTIVE_REQ_TEST59_TREE_LEARNERS', ternary(mode=="quick",80,180));
CFG.MinLeafSize = env_number('ADAPTIVE_REQ_TEST59_MIN_LEAF_SIZE', 10);
CFG.MaxTrainRows = env_number('ADAPTIVE_REQ_TEST59_MAX_TRAIN_ROWS', ternary(mode=="quick",60000,250000));
CFG.ValidateConditions = env_number('ADAPTIVE_REQ_TEST59_VALIDATE_CONDITIONS', 16);
CFG.UseParallel = env_true('ADAPTIVE_REQ_TEST59_USE_PARALLEL', false);
CFG.MaxMapConditions = env_number('ADAPTIVE_REQ_TEST59_MAX_MAP_CONDITIONS', 0);

CFG.RiskThresholdHard = env_number('ADAPTIVE_REQ_TEST59_HARD_RISK_THRESHOLD', 0.20);
CFG.RiskThresholdSelector = env_number('ADAPTIVE_REQ_TEST59_SELECTOR_RISK_THRESHOLD', 0.50);
CFG.PurityThreshold = env_number('ADAPTIVE_REQ_TEST59_PURITY_THRESHOLD', 0.90);
CFG.MixedThreshold = env_number('ADAPTIVE_REQ_TEST59_MIXED_THRESHOLD', 0.25);
CFG.HardSwsRatio = env_number('ADAPTIVE_REQ_TEST59_HARD_SWS_RATIO', 1.05);
CFG.MaxM3RelDiff = env_number('ADAPTIVE_REQ_TEST59_MAX_M3_REL_DIFF', 0.20);
CFG.MaxResidualCorrectionPct = env_number('ADAPTIVE_REQ_TEST59_MAX_RESIDUAL_CORRECTION_PCT', 10);
CFG.LabelGainMarginPct = env_number('ADAPTIVE_REQ_TEST59_LABEL_GAIN_MARGIN_PCT', 0.50);
CFG.AlignToleranceM = env_number('ADAPTIVE_REQ_TEST59_ALIGN_TOLERANCE_M', 0.40e-3);
...
if CFG.QuickMode
    OUT.root_dir = fullfile(root_dir,'outputs','test_59_hard_aware_meff_multiwindow_gate','quick');
else
    OUT.root_dir = fullfile(root_dir,'outputs','test_59_hard_aware_meff_multiwindow_gate');
end
if CFG.ValidateOnly
...
main = ["fixed_M2","fixed_M3","test56_delta_logk_residual_on_M2", ...
    "M2_to_M3_hard_pure_switch","risk_gated_Meff_residual","learned_candidate_selector"];
for s = main
...
export_fig(fig, fullfile(OUT.figure_dir,'test59_hard_aware_summary.png'));
...
export_fig(fig2, fullfile(OUT.figure_dir,'test59_hard_core_by_case.png'));
end
...
fprintf('Saving %d Test59 condition map panels under %s.\n', numel(conds), OUT.map_dir);
for i = 1:numel(conds)
...
strategies = ["fixed_M2","fixed_M3","test56_delta_logk_residual_on_M2", ...
    "learned_candidate_selector"];
...
for s = strategies(1:3)
    X = Tc(strategy_vec==s,:);
    plot_map(nexttile(tl), X, X.sws_signed_error_pct, pretty_strategy(s)+" signed error", 'Error (%)');
end
X = Tc(strategy_vec=="learned_candidate_selector",:);
plot_map(nexttile(tl), X, X.sws_signed_error_pct, 'Selected signed error', 'Error (%)');
...
title(tl, "Test59: " + cond, 'Interpreter','none');
out = fullfile(OUT.map_dir, sanitize_filename(string(X.case_id(1))), ...
    "test59_map__" + sanitize_filename(cond) + ".png");
export_fig(fig, out);
end
...
try
    ...
    warning('Test59:PlotFailed', 'Plot failed (%s)%s: %s', label, loc, ME.message);
end
end
