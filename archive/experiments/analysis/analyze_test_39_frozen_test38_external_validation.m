%% analyze_test_39_frozen_test38_external_validation.m
% Test 39: frozen Test 38 clean spectral-q external validation.
%
% Loads the frozen Test 38 clean spectral-composition q models and evaluates
% them on new held-out simulations. This script never retrains. It is the
% Test 37-style external validation for the broader Test 38 model family.
%
% Runtime controls:
%   ADAPTIVE_REQ_TEST39_MODE          = quick | medium | full
%   ADAPTIVE_REQ_TEST39_VALIDATE_ONLY = true | false
%   ADAPTIVE_REQ_TEST39_SAVE_ALL_MAPS = true | false
%   ADAPTIVE_REQ_TEST39_USE_PARFOR    = true | false
%   ADAPTIVE_REQ_TEST39_MODEL_SOURCE  = medium | full | quick | /path/file.mat
%   ADAPTIVE_REQ_TEST39_DENSE_MAPS    = true | false
%   ADAPTIVE_REQ_TEST39_DENSE_MAP_STEP_M = physical step for diagnostic maps
...
write_config_json(CFG, fullfile(OUT.root_dir,'test39_configuration.json'));

fprintf('\nTest 39: frozen Test 38 external validation\n');
fprintf('Mode: %s | validate only: %d | save all maps: %d\n', ...
    CFG.Mode, CFG.ValidateOnly, CFG.SaveAllMaps);
fprintf('Parallel REQ windows: %d\n', CFG.UseParfor);
fprintf('No training. Frozen model source: %s\n', CFG.ModelSource);
fprintf('At least one source is forced/aligned in every regime.\n');

MODEL_BUNDLE = load_test38_bundle(root_dir, CFG);
MODELS = MODEL_BUNDLE.MODELS;
BASE_FEATURES = string(MODEL_BUNDLE.BASE_FEATURES(:));
assert_no_forbidden_predictors(BASE_FEATURES);

if CFG.ValidateOnly
    validate_test39(root_dir, CFG, MODELS, BASE_FEATURES);
    fprintf('Test 39 validation-only checks passed.\n');
    return;
end

%% Build/load external validation dataset

CASE_SPECS = build_case_specs(CFG);
REGIMES = build_regime_specs(CFG);
fprintf('External cases: %d | regimes: %d | frequencies: %d | M: %d\n', ...
    numel(CASE_SPECS), numel(REGIMES), numel(CFG.Frequencies), numel(CFG.M));

dataset_file = fullfile(OUT.data_dir, 'test39_external_validation_dataset.mat');
if exist(dataset_file,'file') == 2
    S = load(dataset_file, 'T_ext', 'CFG_saved');
    if isfield(S,'CFG_saved') && cache_signature(S.CFG_saved) == cache_signature(CFG)
        T_ext = S.T_ext;
        fprintf('Reused Test 39 external dataset: %d patches.\n', height(T_ext));
    else
        fprintf('Ignoring incompatible Test 39 dataset cache.\n');
        T_ext = build_training_dataset(CASE_SPECS, REGIMES, CFG, OUT);
        CFG_saved = CFG; %#ok<NASGU>
        save(dataset_file, 'T_ext', 'CFG_saved', '-v7.3');
    end
else
    T_ext = build_training_dataset(CASE_SPECS, REGIMES, CFG, OUT);
    CFG_saved = CFG; %#ok<NASGU>
...
assert(~isempty(T_ext), 'Test 39 external dataset is empty.');
T_ext.is_train_row = false(height(T_ext),1);
T_ext.is_heldout_row = true(height(T_ext),1);
fprintf('External patches: %d | conditions: %d | predictors: %d\n', ...
    height(T_ext), numel(unique(T_ext.condition_key)), numel(BASE_FEATURES));

%% Apply frozen model family

T_pred = apply_models(T_ext, MODELS, BASE_FEATURES, "test39_external");
T_held = T_pred;
T_held = add_region_labels(T_held);
T_pred = T_held;
...
T_by_material_region = summarize_predictions(T_held, ["model_name","case_family","material_region"]);
T_by_region_zone = summarize_predictions(T_held, ["model_name","case_family","region_zone"]);
T_by_material_zone = summarize_predictions(T_held, ["model_name","case_id","material_zone"]);
T_by_frequency_region = summarize_predictions(T_held, ["model_name","case_family","material_region","f0"]);
T_by_M_region = summarize_predictions(T_held, ["model_name","case_family","material_region","M"]);
...
writetable(remove_cell_columns(T_pred), fullfile(OUT.table_dir, ...
    'test39_external_patch_level_predictions.csv'));
writetable(T_comp, fullfile(OUT.table_dir, 'test39_composition_metrics.csv'));
writetable(T_overall, fullfile(OUT.table_dir, 'test39_external_summary_overall.csv'));
writetable(T_by_case, fullfile(OUT.table_dir, 'test39_external_summary_by_case.csv'));
writetable(T_by_family, fullfile(OUT.table_dir, 'test39_external_summary_by_family.csv'));
writetable(T_by_regime, fullfile(OUT.table_dir, 'test39_external_summary_by_regime.csv'));
writetable(T_by_frequency, fullfile(OUT.table_dir, 'test39_external_summary_by_frequency.csv'));
writetable(T_by_M, fullfile(OUT.table_dir, 'test39_external_summary_by_M.csv'));
writetable(T_by_purity, fullfile(OUT.table_dir, 'test39_external_summary_by_purity_bin.csv'));
writetable(T_by_material_region, fullfile(OUT.table_dir, 'test39_external_summary_by_material_region.csv'));
writetable(T_by_region_zone, fullfile(OUT.table_dir, 'test39_external_summary_by_region_zone.csv'));
writetable(T_by_material_zone, fullfile(OUT.table_dir, 'test39_external_summary_by_material_zone.csv'));
writetable(T_by_frequency_region, fullfile(OUT.table_dir, 'test39_external_summary_by_frequency_region.csv'));
writetable(T_by_M_region, fullfile(OUT.table_dir, 'test39_external_summary_by_M_region.csv'));
writetable(T_fail, fullfile(OUT.table_dir, 'test39_external_worst_conditions.csv'));

save(fullfile(OUT.data_dir,'test39_external_results.mat'), ...
    'T_pred','T_held','T_comp','T_overall','T_by_case','T_by_family', ...
    'T_by_regime','T_by_frequency','T_by_M','T_by_purity', ...
    'T_by_material_region','T_by_region_zone','T_by_material_zone', ...
    'T_by_frequency_region','T_by_M_region','T_fail','CFG','-v7.3');
...
safe_plot(@() plot_heatmap_table(T_by_case, "case_id", OUT, 'test39_mape_by_case.png'), 'case heatmap');
safe_plot(@() plot_heatmap_table(T_by_regime, "field_regime_ood", OUT, 'test39_mape_by_regime.png'), 'regime heatmap');
safe_plot(@() plot_heatmap_table(T_by_frequency, "f0", OUT, 'test39_mape_by_frequency.png'), 'frequency heatmap');
safe_plot(@() plot_heatmap_table(T_by_M, "M", OUT, 'test39_mape_by_M.png'), 'M heatmap');
safe_plot(@() plot_error_vs_predicted_purity(T_held, OUT), 'error vs predicted purity');
safe_plot(@() plot_region_diagnostics(T_by_material_region, T_by_region_zone, ...
    T_by_material_zone, OUT), 'regional diagnostics');
safe_plot(@() plot_mape_vs_m_frequency(T_by_M_region, T_by_frequency_region, OUT), ...
    'MAPE vs M/frequency');
safe_plot(@() plot_sws_vs_m_frequency(T_held, OUT), 'SWS vs M/frequency');
if CFG.SaveAllMaps
    if CFG.DenseDiagnosticMaps
        safe_plot(@() plot_dense_diagnostic_maps(T_held, CASE_SPECS, REGIMES, ...
            CFG, OUT, MODELS, BASE_FEATURES), 'dense condition maps');
    else
...
mode = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST39_MODE'))));
if mode == "", mode = "quick"; end
assert(ismember(mode, ["quick","medium","full"]), ...
    'ADAPTIVE_REQ_TEST39_MODE must be quick, medium, or full.');
...
CFG.ValidateOnly = env_true('ADAPTIVE_REQ_TEST39_VALIDATE_ONLY', false);
CFG.SaveAllMaps = env_true('ADAPTIVE_REQ_TEST39_SAVE_ALL_MAPS', true);
CFG.RandomSeed = 39001;
CFG.dx = 0.2e-3;
CFG.dz = 0.2e-3;
CFG.Lx = 0.05;
CFG.Lz = 0.05;
...
CFG.UseParfor = env_true('ADAPTIVE_REQ_TEST39_USE_PARFOR', false);
CFG.UseParallelTraining = false;
CFG.ParallelPoolType = "threads";
CFG.TrainFraction = 0.70;
CFG.MaxPatchesPerCondition = 300;
if CFG.QuickMode
    CFG.MaxPatchesPerCondition = 550;
end
CFG.DenseDiagnosticMaps = env_true('ADAPTIVE_REQ_TEST39_DENSE_MAPS', true);
CFG.DenseMapTargetStepM = env_number('ADAPTIVE_REQ_TEST39_DENSE_MAP_STEP_M', 0.6e-3);
CFG.DiagnosticMapInterpScale = env_number('ADAPTIVE_REQ_TEST39_MAP_INTERP_SCALE', 4);
CFG.MapStyle = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST39_MAP_STYLE'))));
if CFG.MapStyle == "", CFG.MapStyle = "patch"; end
assert(ismember(CFG.MapStyle, ["patch","interp"]), ...
    'ADAPTIVE_REQ_TEST39_MAP_STYLE must be patch or interp.');
CFG.ModelSource = string(getenv('ADAPTIVE_REQ_TEST39_MODEL_SOURCE'));
if CFG.ModelSource == "", CFG.ModelSource = "medium"; end
...
elseif CFG.MediumMode
    CFG.Frequencies = [375 575];
    CFG.M = 2;
    CFG.RegimeSet = "external_medium";
    CFG.TreeLearners = 140;
else
    CFG.Frequencies = [325 425 525 625];
    CFG.M = [2 3];
    CFG.RegimeSet = "external_full";
end
end

function OUT = make_output_dirs(root_dir, CFG)
OUT.root_dir = fullfile(root_dir, 'outputs', 'test_39_frozen_test38_external_validation');
if CFG.QuickMode
...
OUT.dense_map_condition_dir = fullfile(OUT.data_dir, 'dense_map_features');
...
OUT.dense_map_dir = fullfile(OUT.figure_dir, 'dense_maps_by_condition');
...
assert(exist(file,'file') == 2, ...
    'Frozen Test 38 model bundle not found: %s', file);
S = load(file, 'MODEL_BUNDLE');
B = S.MODEL_BUNDLE;
fprintf('Loaded frozen Test 38 model bundle: %s\n', file);
end

function validate_test39(root_dir, CFG, MODELS, BASE_FEATURES)
...
assert_no_forbidden_predictors(BASE_FEATURES);
T.is_train_row = false(height(T),1);
T.is_heldout_row = true(height(T),1);
P = apply_models(T, MODELS, BASE_FEATURES, "validate");
assert(~isempty(T) && all(isfinite(T.q_oracle)));
assert(~isempty(P) && all(isfinite(P.sws_pred)));
fprintf('Validation sample: %s, %d patches, %d clean predictors.\n', ...
    C(1).case_id, height(T), numel(BASE_FEATURES));
end

function T_all = build_training_dataset(CASE_SPECS, REGIMES, CFG, OUT)
parts = {};
condition_id = 0;
total = numel(CASE_SPECS) * numel(REGIMES) * numel(CFG.Frequencies) * numel(CFG.M);
        ...
        for ri = 1:numel(REGIMES)
            ...
            for mi = 1:numel(CFG.M)
                ...
                if exist(feature_file,'file') == 2
                    S = load(feature_file, 'T_condition');
                    T_condition = S.T_condition;
                    fprintf('[%d/%d] reused %s (%d patches).\n', ...
                        numel(parts)+1, total, key, height(T_condition));
                else
                    timer = tic;
                    T_condition = extract_training_condition(sim, cfg_sim, C, R, M, key, CFG);
                    save(feature_file, 'T_condition', '-v7.3');
                    fprintf('[%d/%d] built %s (%d patches) in %.1f s.\n', ...
                        numel(parts)+1, total, key, height(T_condition), toc(timer));
                end
...
T_all = vertcat(parts{:});
T_all.dataset = repmat("test39_external", height(T_all), 1);
...
base = [
    homogeneous_case("ood_homogeneous_cs1p75", 1.75)
    homogeneous_case("ood_homogeneous_cs2p25", 2.25)
    homogeneous_case("ood_homogeneous_cs2p75", 2.75)
    homogeneous_case("ood_homogeneous_cs3p25", 3.25)
    homogeneous_case("ood_homogeneous_cs3p75", 3.75)
    bilayer_case("ood_bilayer_1p75_3p25", 1.75, 3.25, 0)
    bilayer_case("ood_bilayer_2p25_3p75", 2.25, 3.75, 0)
    bilayer_case("ood_oblique_bilayer_1p75_3p75", 1.75, 3.75, deg2rad(37))
    bilayer_case("ood_oblique_bilayer_2p25_3p25", 2.25, 3.25, deg2rad(-18))
    inclusion_case("ood_inclusion_1p75_3p25", 1.75, 3.25, [0.025 0.025], 0.0075)
    inclusion_case("ood_inclusion_2p25_3p75", 2.25, 3.75, [0.025 0.025], 0.0090)
    ellipse_case("ood_ellipse_1p75_3p75", 1.75, 3.75, [0.026 0.024], [0.015 0.005], deg2rad(-32))
    inclusion_case("ood_offcenter_inclusion_1p75_3p25", 1.75, 3.25, [0.017 0.031], 0.0065)
    two_inclusion_case("ood_two_inclusions_1p75_3p75", 1.75, 3.75)
    thin_layer_case("ood_thin_layer_1p75_3p75", 1.75, 3.75)
    three_material_case("ood_three_material_1p75_2p75_3p75", 1.75, 2.75, 3.75)
    smooth_bilayer_case("ood_smooth_bilayer_1p75_3p75", 1.75, 3.75, 1.8e-3)
    ];
if CFG.QuickMode
    keep = ["ood_homogeneous_cs1p75","ood_homogeneous_cs3p75", ...
        "ood_bilayer_1p75_3p25","ood_inclusion_2p25_3p75", ...
        "ood_two_inclusions_1p75_3p75","ood_three_material_1p75_2p75_3p75"];
...
if CFG.QuickMode
    R = [
        regime("directional_2D_angle15", "directional_2D", true, 1, true, ...
        "ranges", "fibonacci", [pi/12 pi/12], [0 2*pi], [0 pi], 101)
        regime("directional_2D_angle75", "directional_2D", true, 1, true, ...
        "ranges", "fibonacci", [5*pi/12 5*pi/12], [0 2*pi], [0 pi], 103)
        regime("diffuse_2D_seed7", "diffuse_2D", true, 128, true, ...
        "ranges", "random", [0 2*pi], [0 2*pi], [0 pi], 107)
        regime("diffuse_3D_seed7", "diffuse_3D", false, 128, true, ...
        "ranges", "random", [0 2*pi], [0 2*pi], [0 pi], 109)
        regime("partial_3D_12src", "partial_3D", false, 12, true, ...
        "ranges", "fibonacci", [0 2*pi], [0 2*pi], [0 pi], 113)
        ];
else
    R = [
        regime("directional_2D_angle15", "directional_2D", true, 1, true, ...
        ...
        "ranges", "fibonacci", [pi/4 pi/4], [0 2*pi], [0 pi], 103)
        regime("directional_2D_angle75", "directional_2D", true, 1, true, ...
        "ranges", "fibonacci", [5*pi/12 5*pi/12], [0 2*pi], [0 pi], 107)
        regime("diffuse_2D_seed7", "diffuse_2D", true, 128, true, ...
        "ranges", "random", [0 2*pi], [0 2*pi], [0 pi], 109)
        regime("diffuse_2D_seed8", "diffuse_2D", true, 96, true, ...
        "ranges", "random", [0 2*pi], [0 2*pi], [0 pi], 113)
        regime("diffuse_3D_seed7", "diffuse_3D", false, 128, true, ...
        "ranges", "random", [0 2*pi], [0 2*pi], [0 pi], 127)
        regime("diffuse_3D_seed8", "diffuse_3D", false, 96, true, ...
        "ranges", "random", [0 2*pi], [0 2*pi], [0 pi], 131)
        regime("partial_3D_12src", "partial_3D", false, 12, true, ...
        "ranges", "fibonacci", [0 2*pi], [0 2*pi], [0 pi], 137)
        regime("partial_3D_24src", "partial_3D", false, 24, true, ...
        "ranges", "fibonacci", [0 2*pi], [0 2*pi], [0 pi], 139)
        ];
end
end
...
function T = add_region_labels(T)
T.material_region = repmat("unknown", height(T), 1);
T.region_zone = repmat("unknown", height(T), 1);
T.material_zone = repmat("unknown", height(T), 1);
keys = unique(T.condition_key, 'stable');
for ki = 1:numel(keys)
    idx = T.condition_key == keys(ki);
    X = T(idx,:);
    cs = X.true_SWS;
    ...
    cmax = max(cs, [], 'omitnan');
    crange = cmax - cmin;
    if ~isfinite(crange) || crange < 0.05
        mat = repmat("homogeneous", height(X), 1);
    elseif contains(fam, ["inclusion","ellipse","two_inclusions"])
        mat = repmat("background", height(X), 1);
        mat(cs >= cmin + 0.5*crange) = "inclusion";
    elseif contains(fam, "three_material")
        mat = repmat("soft", height(X), 1);
    ...
    zone = repmat("near_interface_2_4mm", height(X), 1);
    zone(~isfinite(d)) = "homogeneous_core";
    zone(d <= 2) = "interface_0_2mm";
    zone(d > 4) = "core_gt4mm";
    mz = mat + "_" + zone;
    T.material_region(idx) = mat;
    T.region_zone(idx) = zone;
    T.material_zone(idx) = mz;
end
end
...
T_overall = plot_models_only(T_overall);
T_family = plot_models_only(T_family);
T_purity = plot_models_only(T_purity);
...
export_fig(fig, fullfile(OUT.figure_dir,'test39_summary_bars.png'));
end

function plot_heatmap_table(T, col, OUT, name)
T = plot_models_only(T);
...
keep = ismember(T.model_name, ["q_spectrum_only","delta_logk_theory_composition", ...
...
title('Held-out error vs predicted composition','FontWeight','normal', ...
    'Interpreter','none');
grid on; colorbar;
export_fig(fig, fullfile(OUT.figure_dir,'test39_error_vs_predicted_purity.png'));
end
...
grid(ax,'on');
export_fig(fig, fullfile(OUT.figure_dir,'test39_composition_diagnostics.png'));
end
...
title('Test 39 frozen external model ranking','Interpreter','none','FontWeight','normal');
export_fig(fig, fullfile(OUT.figure_dir,'test39_model_ranking.png'));
end

function plot_dense_diagnostic_maps(T_sparse, CASE_SPECS, REGIMES, CFG, OUT, MODELS, BASE_FEATURES)
keys = map_keys_to_plot(T_sparse, CFG);
if isempty(keys)
    fprintf('No dense diagnostic map keys selected.\n');
    return;
end
CFG_dense = CFG;
CFG_dense.TargetStepM = CFG.DenseMapTargetStepM;
CFG_dense.MaxPatchesPerCondition = Inf;
OUT_dense = OUT;
OUT_dense.map_dir = OUT.dense_map_dir;
fprintf('Building dense diagnostic maps for %d conditions (step %.3f mm).\n', ...
    numel(keys), 1e3*CFG_dense.TargetStepM);

parts = cell(numel(keys),1);
for ki = 1:numel(keys)
    key = keys(ki);
    X = T_sparse(T_sparse.condition_key == key, :);
    X = X(1,:);
    C = CASE_SPECS([CASE_SPECS.case_id] == X.case_id);
    R = REGIMES([REGIMES.regime_id] == X.field_regime_ood);
    if isempty(C) || isempty(R)
        warning('Skipping dense map for %s: case/regime metadata not found.', key);
        continue;
    end
    C = C(1); R = R(1);
    f0 = X.f0(1); M = X.M(1);
    ...
    if exist(field_file,'file') == 2
        ...
        cfg_sim = build_sim_cfg(CFG, C, R, f0, 390000 + ki);
        sim = run_ood_simulation(cfg_sim, C);
        case_spec = C; regime_spec = R; %#ok<NASGU>
        save(field_file, 'sim', 'cfg_sim', 'case_spec', 'regime_spec', '-v7.3');
    end
    dense_key = string(field_key) + "__M" + string(M);
    dense_tag = sprintf('__dense_step%gum', round(1e6*CFG_dense.TargetStepM));
    feature_file = fullfile(OUT.dense_map_condition_dir, ...
        "features__" + sanitize(dense_key + dense_tag) + ".mat");
    if exist(feature_file,'file') == 2
        S = load(feature_file, 'T_condition');
        T_condition = S.T_condition;
        fprintf('[dense %d/%d] reused %s (%d patches).\n', ...
            ki, numel(keys), dense_key, height(T_condition));
    else
        timer = tic;
        T_condition = extract_training_condition(sim, cfg_sim, C, R, M, dense_key, CFG_dense);
        save(feature_file, 'T_condition', '-v7.3');
        fprintf('[dense %d/%d] built %s (%d patches) in %.1f s.\n', ...
            ki, numel(keys), dense_key, height(T_condition), toc(timer));
    end
    T_condition.is_train_row = false(height(T_condition),1);
    T_condition.is_heldout_row = true(height(T_condition),1);
    T_map = apply_models(T_condition, MODELS, BASE_FEATURES, "test39_dense_map");
    T_map = add_region_labels(T_map);
    parts{ki} = T_map;
end
parts = parts(~cellfun(@isempty, parts));
if isempty(parts)
    warning('Dense diagnostic map construction produced no tables.');
    return;
end
T_maps = vertcat(parts{:});
plot_training_maps(T_maps, OUT_dense, CFG);
end

function keys = map_keys_to_plot(T, CFG)
keys = unique(T.condition_key, 'stable');
if ~CFG.QuickMode
...
preferred = ["q_spectrum_only","q_spectrum_plus_composition", ...
    "q_spectrum_plus_theory_composition","delta_logk_theory_composition"];
keys = map_keys_to_plot(T, CFG);
...
for ki = 1:numel(keys)
    ...
    for mi = 1:min(numel(preferred),4)
    ...
    out_dir = fullfile(OUT.map_dir, sanitize(X0.case_id(1)), ...
        ...
        "M" + string(X0.M(1)));
    if exist(out_dir,'dir') ~= 7, mkdir(out_dir); end
    out_file = fullfile(out_dir, "test39_map__" + sanitize(keys(ki)) + ".png");
    export_fig(fig, out_file);
end
end

function plot_region_diagnostics(T_material, T_zone, T_material_zone, OUT)
T_material = plot_models_only(T_material);
T_zone = plot_models_only(T_zone);
T_material_zone = plot_models_only(T_material_zone);
fig = figure('Color','w','Units','centimeters','Position',[1 1 32 18]);
tl = tiledlayout(fig,2,2,'TileSpacing','compact','Padding','compact');
ax = nexttile(tl);
plot_group_heatmap(ax, T_material, "material_region", "MAPE_pct", ...
    'MAPE by material region');
ax = nexttile(tl);
plot_group_heatmap(ax, T_material, "material_region", "mean_signed_error_pct", ...
    'Signed error by material region');
ax = nexttile(tl);
core = T_material_zone(contains(string(T_material_zone.material_zone), "core"),:);
plot_group_heatmap(ax, core, "material_zone", "MAPE_pct", ...
    'Core ROI MAPE by material');
ax = nexttile(tl);
plot_group_heatmap(ax, T_zone, "region_zone", "MAPE_pct", ...
    'MAPE by interface/core zone');
export_fig(fig, fullfile(OUT.figure_dir,'test39_region_error_diagnostics.png'));
end

function plot_mape_vs_m_frequency(T_M, T_F, OUT)
T_M = plot_models_only(T_M);
T_F = plot_models_only(T_F);
...
hold(ax,'on');
for m = models
    X = T_M(T_M.model_name == m,:);
    if isempty(X), continue; end
    [G, gM] = findgroups(X.M);
    y = splitapply(@(v) mean(v,'omitnan'), X.MAPE_pct, G);
    plot(ax, gM, y, '-o', 'DisplayName', char(m), 'LineWidth', 1.1);
end
xlabel(ax,'REQ M'); ylabel(ax,'MAPE (%)'); title(ax,'MAPE vs M', ...
    'Interpreter','none','FontWeight','normal'); grid(ax,'on');
legend(ax,'Interpreter','none','Location','best');
ax = nexttile(tl);
hold(ax,'on');
for m = models
    X = T_F(T_F.model_name == m,:);
    if isempty(X), continue; end
    [G, gF] = findgroups(X.f0);
    y = splitapply(@(v) mean(v,'omitnan'), X.MAPE_pct, G);
    plot(ax, gF, y, '-o', 'DisplayName', char(m), 'LineWidth', 1.1);
end
xlabel(ax,'Frequency (Hz)'); ylabel(ax,'MAPE (%)'); title(ax,'MAPE vs frequency', ...
    'Interpreter','none','FontWeight','normal'); grid(ax,'on');
legend(ax,'Interpreter','none','Location','best');
export_fig(fig, fullfile(OUT.figure_dir,'test39_mape_vs_M_frequency.png'));
end

function plot_sws_vs_m_frequency(T, OUT)
T = plot_models_only(T);
models = ["q_spectrum_only","q_spectrum_plus_composition", ...
    "q_spectrum_plus_theory_composition","delta_logk_theory_composition"];
core = T(T.region_zone == "core_gt4mm" | T.region_zone == "homogeneous_core",:);
fig = figure('Color','w','Units','centimeters','Position',[1 1 32 18]);
tl = tiledlayout(fig,2,2,'TileSpacing','compact','Padding','compact');
regions = ["background","inclusion","soft","hard"];
for ri = 1:numel(regions)
    ax = nexttile(tl); hold(ax,'on');
    for m = models
        X = core(core.model_name == m & core.material_region == regions(ri),:);
        if isempty(X), continue; end
        [G, gF] = findgroups(X.f0);
        y = splitapply(@(v) mean(v,'omitnan'), X.sws_pred, G);
        e = splitapply(@(v) std_omitnan(v), X.sws_pred, G);
        errorbar(ax, gF, y, e, '-o', 'DisplayName', char(m), 'LineWidth', 1.0);
    end
    xlabel(ax,'Frequency (Hz)'); ylabel(ax,'Predicted SWS (m/s)');
    title(ax, "Core predicted SWS: " + regions(ri), ...
        'Interpreter','none','FontWeight','normal');
    grid(ax,'on'); legend(ax,'Interpreter','none','Location','best');
end
export_fig(fig, fullfile(OUT.figure_dir,'test39_core_sws_vs_frequency.png'));
...
for ri = 1:numel(regions)
    ...
    for m = models
        ...
        y = splitapply(@(v) mean(v,'omitnan'), X.sws_pred, G);
        e = splitapply(@(v) std_omitnan(v), X.sws_pred, G);
        errorbar(ax, gM, y, e, '-o', 'DisplayName', char(m), 'LineWidth', 1.0);
    end
    xlabel(ax,'REQ M'); ylabel(ax,'Predicted SWS (m/s)');
    title(ax, "Core predicted SWS: " + regions(ri), ...
        'Interpreter','none','FontWeight','normal');
    grid(ax,'on'); legend(ax,'Interpreter','none','Location','best');
end
export_fig(fig, fullfile(OUT.figure_dir,'test39_core_sws_vs_M.png'));
end

function T = plot_models_only(T)
if ismember("model_name", string(T.Properties.VariableNames))
    T = T(T.model_name ~= "theory_discrete",:);
end
end
...
fprintf('\n================ Test 39 frozen external summary ================\n');
...
try
    ...
    warning('Test38:PlotFailed', 'Plot failed (%s): %s', label, ME.message);
end
end
...
for f = ["SaveAllMaps","DenseDiagnosticMaps","DenseMapTargetStepM", ...
        "DiagnosticMapInterpScale","MapStyle"]
...
if isfield(CFG, 'MapStyle') && CFG.MapStyle == "patch"
    if nargin < 5
        [Z,nz,nx] = rows_to_grid(T, values);
    else
...
if exist('gcp','file') ~= 2 || exist('parpool','file') ~= 2
    warning('Test38:NoParallelToolbox', ...
...
try
    ...
    warning('Test38:ParallelPoolFailed', ...
