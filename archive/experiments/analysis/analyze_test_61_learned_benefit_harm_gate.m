%% analyze_test_61_learned_benefit_harm_gate.m
% Test 61: learned benefit/harm correction gate.
%
...
write_config_json(CFG, fullfile(OUT.root_dir,'test61_configuration.json'));
...
assert_no_forbidden_predictors([CFG.ResidualFeatureVars; CFG.RiskFeatureVars; ...
    CFG.SelectorFeatureVars; CFG.CandidateFeatureVars]);
...
assert(any(abs(Mvals-2)<1e-8), 'Test61 requires M=2 rows.');
assert(any(abs(Mvals-3)<1e-8), 'Test61 requires M=3 rows. Rerun Test55 with ADAPTIVE_REQ_TEST55_M_LIST including 3.');
...
writetable(remove_cell_columns(T_strategies), fullfile(OUT.table_dir,'test61_patch_level_results.csv'));
writetable(T_overall, fullfile(OUT.table_dir,'test61_strategy_summary_overall.csv'));
writetable(T_by_case, fullfile(OUT.table_dir,'test61_strategy_summary_by_case.csv'));
writetable(T_by_freq, fullfile(OUT.table_dir,'test61_strategy_summary_by_frequency.csv'));
writetable(T_by_regime, fullfile(OUT.table_dir,'test61_strategy_summary_by_regime.csv'));
writetable(T_by_roi, fullfile(OUT.table_dir,'test61_strategy_summary_by_roi.csv'));
writetable(T_by_case_roi, fullfile(OUT.table_dir,'test61_strategy_summary_by_case_roi.csv'));
writetable(T_harm, fullfile(OUT.table_dir,'test61_correction_harm_summary.csv'));
writetable(T_success, fullfile(OUT.table_dir,'test61_success_criteria_summary.csv'));
...
save(fullfile(OUT.data_dir,'test61_learned_benefit_harm_gate_results.mat'), ...
...
mode = lower(strtrim(string(getenv('ADAPTIVE_REQ_TEST61_MODE'))));
if mode == "", mode = "quick"; end
assert(ismember(mode, ["quick","full"]), 'ADAPTIVE_REQ_TEST61_MODE must be quick or full.');
CFG = struct();
CFG.Mode = mode;
CFG.QuickMode = mode == "quick";
CFG.ValidateOnly = env_true('ADAPTIVE_REQ_TEST61_VALIDATE_ONLY', false);
CFG.SaveAllMaps = env_true('ADAPTIVE_REQ_TEST61_SAVE_ALL_MAPS', true);
CFG.RandomSeed = 59001;
CFG.SourceCsv = env_string('ADAPTIVE_REQ_TEST61_SOURCE_CSV', ...
    fullfile(root_dir,'outputs','test_55_controlled_field_frequency_roi_sweep', ...
    'tables','test55_patch_level_results.csv'));
CFG.BaseModel = env_string('ADAPTIVE_REQ_TEST61_BASE_MODEL', ...
    "q_spectrum_plus_composition");
CFG.TrainFraction = env_number('ADAPTIVE_REQ_TEST61_TRAIN_FRACTION', 0.70);
CFG.CsGuess = env_number('ADAPTIVE_REQ_TEST61_CS_GUESS', 3.0);
CFG.TreeLearners = env_number('ADAPTIVE_REQ_TEST61_TREE_LEARNERS', ternary(mode=="quick",80,180));
CFG.MinLeafSize = env_number('ADAPTIVE_REQ_TEST61_MIN_LEAF_SIZE', 10);
CFG.MaxTrainRows = env_number('ADAPTIVE_REQ_TEST61_MAX_TRAIN_ROWS', ternary(mode=="quick",60000,250000));
CFG.ValidateConditions = env_number('ADAPTIVE_REQ_TEST61_VALIDATE_CONDITIONS', 16);
CFG.UseParallel = env_true('ADAPTIVE_REQ_TEST61_USE_PARALLEL', false);
CFG.MaxMapConditions = env_number('ADAPTIVE_REQ_TEST61_MAX_MAP_CONDITIONS', 0);

CFG.RiskThresholdHard = env_number('ADAPTIVE_REQ_TEST61_HARD_RISK_THRESHOLD', 0.20);
CFG.RiskThresholdSelector = env_number('ADAPTIVE_REQ_TEST61_SELECTOR_RISK_THRESHOLD', 0.50);
CFG.PurityThreshold = env_number('ADAPTIVE_REQ_TEST61_PURITY_THRESHOLD', 0.90);
CFG.MixedThreshold = env_number('ADAPTIVE_REQ_TEST61_MIXED_THRESHOLD', 0.25);
CFG.HardSwsRatio = env_number('ADAPTIVE_REQ_TEST61_HARD_SWS_RATIO', 1.05);
CFG.MaxM3RelDiff = env_number('ADAPTIVE_REQ_TEST61_MAX_M3_REL_DIFF', 0.20);
CFG.MaxResidualCorrectionPct = env_number('ADAPTIVE_REQ_TEST61_MAX_RESIDUAL_CORRECTION_PCT', 10);
CFG.LabelGainMarginPct = env_number('ADAPTIVE_REQ_TEST61_LABEL_GAIN_MARGIN_PCT', 0.50);
CFG.AlignToleranceM = env_number('ADAPTIVE_REQ_TEST61_ALIGN_TOLERANCE_M', 0.40e-3);
CFG.InterfaceRiskThreshold = env_number('ADAPTIVE_REQ_TEST61_INTERFACE_RISK_THRESHOLD', 0.30);
CFG.InterfaceMixedThreshold = env_number('ADAPTIVE_REQ_TEST61_INTERFACE_MIXED_THRESHOLD', 0.50);
CFG.InterfacePurityThreshold = env_number('ADAPTIVE_REQ_TEST61_INTERFACE_PURITY_THRESHOLD', 0.80);
CFG.InterfaceExtraGainMarginPct = env_number('ADAPTIVE_REQ_TEST61_INTERFACE_EXTRA_GAIN_MARGIN_PCT', 2.0);
CFG.PenalizedSelectorRiskThreshold = env_number('ADAPTIVE_REQ_TEST61_PENALIZED_SELECTOR_RISK_THRESHOLD', 0.35);
CFG.SpatialGateRadiusPx = env_number('ADAPTIVE_REQ_TEST61_SPATIAL_GATE_RADIUS_PX', 1);
CFG.SpatialGateMinSupport = env_number('ADAPTIVE_REQ_TEST61_SPATIAL_GATE_MIN_SUPPORT', 4);
CFG.HarmMarginPct = env_number('ADAPTIVE_REQ_TEST61_HARM_MARGIN_PCT', 0.50);
CFG.BenefitMinGainPct = env_number('ADAPTIVE_REQ_TEST61_BENEFIT_MIN_GAIN_PCT', 0.50);
CFG.BenefitMaxHarmProb = env_number('ADAPTIVE_REQ_TEST61_BENEFIT_MAX_HARM_PROB', 0.05);
CFG.BenefitHarmPenaltyPct = env_number('ADAPTIVE_REQ_TEST61_BENEFIT_HARM_PENALTY_PCT', 20);
CFG.BenefitMaxCandidateRisk = env_number('ADAPTIVE_REQ_TEST61_BENEFIT_MAX_CANDIDATE_RISK', 0.50);
CFG.ConservativeMinGainPct = env_number('ADAPTIVE_REQ_TEST61_CONSERVATIVE_MIN_GAIN_PCT', 1.00);
CFG.ConservativeMaxHarmProb = env_number('ADAPTIVE_REQ_TEST61_CONSERVATIVE_MAX_HARM_PROB', 0.025);
CFG.ConservativeHarmPenaltyPct = env_number('ADAPTIVE_REQ_TEST61_CONSERVATIVE_HARM_PENALTY_PCT', 30);
CFG.ConservativeMaxCandidateRisk = env_number('ADAPTIVE_REQ_TEST61_CONSERVATIVE_MAX_CANDIDATE_RISK', 0.35);
...
CFG.CandidateFeatureVars = ["candidate_code";"candidate_q";"candidate_sws"; ...
    "candidate_log_sws";"candidate_k";"candidate_logk";"candidate_M_eff"; ...
    "candidate_source_M";"candidate_risk_high20";"candidate_delta_sws"; ...
    "candidate_rel_delta_sws";"candidate_abs_rel_delta_sws"; ...
    "M2_q";"M2_sws";"M2_M_eff";"M2_risk_high20"; ...
    "M2_predicted_patch_purity";"M2_p_mixed";"M2_p_strong_mixed"; ...
    "M3_sws";"M3_M_eff";"sws_M3_minus_M2";"rel_sws_M3_minus_M2"; ...
    "f0";"dx";"dz";"field_code";"case_family_code"];
end

function OUT = make_output_dirs(root_dir, CFG)
if CFG.QuickMode
    OUT.root_dir = fullfile(root_dir,'outputs','test_61_learned_benefit_harm_gate','quick');
else
    OUT.root_dir = fullfile(root_dir,'outputs','test_61_learned_benefit_harm_gate');
end
if CFG.ValidateOnly
...
MODEL.benefit_harm = train_benefit_harm_models(W_with_risk, train_mask, ...
    MODEL.residual, MODEL.risk, CFG);
end
...
function BH = train_benefit_harm_models(W, train_mask, residual_model, risk_model, CFG)
delta = predict(residual_model, W(:, cellstr(CFG.ResidualFeatureVars)));
delta = clamp(delta, -0.45, 0.45);
sws_resid = apply_delta_logk(W.M2_k, W.f0, delta);
[C, M] = build_candidate_feature_table(W, risk_model, sws_resid, delta, CFG);

base_err = abs(100*(W.M2_sws - W.true_SWS)./W.true_SWS);
candidate_err = M.candidate_abs_error_pct;
gain = base_err(M.row_idx) - candidate_err;
harm_label = candidate_err > base_err(M.row_idx) + CFG.HarmMarginPct;

X = C(:, cellstr(CFG.CandidateFeatureVars));
valid = train_mask(M.row_idx) & all(isfinite(table2array(X)),2) & isfinite(gain);
idx = downsample_idx(find(valid), CFG.MaxTrainRows, CFG.RandomSeed + 37);
template = templateTree('MinLeafSize', CFG.MinLeafSize);
opts = statset('UseParallel', CFG.UseParallel);

BH.gain_model = fitrensemble(X(idx,:), gain(idx), ...
    'Method','Bag','NumLearningCycles',CFG.TreeLearners, ...
    'Learners',template,'Options',opts);
BH.harm_model = fitcensemble(X(idx,:), logical(harm_label(idx)), ...
    'Method','Bag','NumLearningCycles',CFG.TreeLearners, ...
    'Learners',template,'Options',opts);
BH.feature_vars = CFG.CandidateFeatureVars;
fprintf('Trained benefit/harm gate models on %d candidate rows.\n', numel(idx));
end
...
% Avoid accepting corrections in operationally interface-like pixels unless
% the learned benefit is large enough to compensate for the extra risk.
interface_like = operational_interface_like(W, CFG);
hard_like = operational_hard_pure_like(W, CFG);
...
n = height(W);
row_idx = [(1:n)'; (1:n)'];
candidate_code = [ones(n,1); 2*ones(n,1)]; % 1=M3, 2=residual
...
main = ["fixed_M2","fixed_M3","test56_delta_logk_residual_on_M2", ...
    ...
    "spatial_interface_penalized_selector","learned_benefit_harm_gate", ...
    "conservative_benefit_harm_gate"];
for s = main
...
export_fig(fig, fullfile(OUT.figure_dir,'test61_hard_aware_summary.png'));
...
export_fig(fig2, fullfile(OUT.figure_dir,'test61_hard_core_by_case.png'));
end
...
fprintf('Saving %d Test61 condition map panels under %s.\n', numel(conds), OUT.map_dir);
for i = 1:numel(conds)
...
strategies = ["fixed_M2","learned_candidate_selector","interface_penalized_selector", ...
...
for s = ["fixed_M2","interface_penalized_selector","learned_benefit_harm_gate", ...
        "conservative_benefit_harm_gate"]
    X = Tc(strategy_vec==s,:);
    plot_map(nexttile(tl), X, X.sws_signed_error_pct, pretty_strategy(s)+" signed error", 'Error (%)');
end
X = Tc(strategy_vec=="conservative_benefit_harm_gate",:);
...
title(tl, "Test61: " + cond, 'Interpreter','none');
out = fullfile(OUT.map_dir, sanitize_filename(string(X.case_id(1))), ...
    "test61_map__" + sanitize_filename(cond) + ".png");
export_fig(fig, out);
end
...
s(s=="learned_benefit_harm_gate") = "Learned benefit/harm gate";
s(s=="conservative_benefit_harm_gate") = "Conservative benefit/harm gate";
...
try
    ...
    warning('Test61:PlotFailed', 'Plot failed (%s)%s: %s', label, loc, ME.message);
end
end
