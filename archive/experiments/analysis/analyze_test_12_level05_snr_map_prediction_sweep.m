%% analyze_test_12_level05_snr_map_prediction_sweep.m
% Test 12 Level 05: map-level adaptive-q REQ SNR diagnostics.
%
% This script runs new homogeneous simulations over frequency, REQ window
% size, true background SWS, aperture, and SNR. It applies the trained Test
% 12 adaptive-q model to produce SWS maps, error maps, q maps, center-ROI
% MAPE, and center-ROI CoV.
...
CFG = adaptive_req.config.load_profile_config( ...
    'test_12_level05_snr_map_prediction_sweep', ...
    'RootDir', root_dir);
...
analysis_tag = sprintf('level_05_snr_map_prediction_sweep_%s_%s', ...
    sanitize_filename(CFG.MAP05.MODEL.ModelName), ...
    sanitize_filename(CFG.MAP05.MODEL.FeatureSet));
...
checkpoint_file = fullfile(data_dir, ...
    'level12_level05_map_prediction_checkpoint.mat');

%% Build condition table

T_conditions = build_condition_table(CFG);
fprintf('\nLevel 05 condition count: %d\n', height(T_conditions));
fprintf('SNR values: %s dB\n', strjoin(string(arrayfun( ...
    @snr_label, CFG.MAP05.SNR_list, 'UniformOutput', false)), ', '));
...
for ci = 1:height(T_conditions)
    ...
    fprintf('\n=== Level 05 map condition %03d / %03d ===\n', ...
        ci, height(T_conditions));
    disp(cond(:, {'SIM_f0', 'REQ_M', 'SIM_cs_bg', ...
        'SIM_SNR_dB', 'SNR_label', 'aperture_label', ...
        'ConeHalfAngleDeg'}));
    ...
    fprintf('Extracting REQ map features with StepX=%d, StepZ=%d...\n', ...
        CFG.MAP05.REQ.StepX, CFG.MAP05.REQ.StepZ);
    t_feat = tic;
    ...
    save(fullfile(condition_dir, sprintf( ...
        'level12_level05_condition_%03d.mat', cond.condition_id)), ...
        'CFG', 'cfg', 'cond', 'T_feat', 'T_pred_cond', ...
        'T_roi_cond', 'T_overall_cond', 'req_out', 'global_req', '-v7.3');

    if CFG.MAP05.FIG.SavePerConditionMaps
...
writetable(remove_cell_columns(T_all_pred), fullfile(table_dir, ...
    'level12_level05_map_predictions.csv'));
writetable(T_all_roi, fullfile(table_dir, ...
    'level12_level05_center_roi_metrics.csv'));
writetable(T_all_overall, fullfile(table_dir, ...
    'level12_level05_overall_map_metrics.csv'));
writetable(T_conditions, fullfile(table_dir, ...
    'level12_level05_condition_table.csv'));

T_snr_roi = summarize_snr_metrics(T_all_roi);
T_snr_overall = summarize_snr_metrics(T_all_overall);
writetable(T_snr_roi, fullfile(table_dir, ...
    'level12_level05_center_roi_metrics_by_snr.csv'));
writetable(T_snr_overall, fullfile(table_dir, ...
    'level12_level05_overall_map_metrics_by_snr.csv'));

save(fullfile(data_dir, 'level12_level05_map_prediction_sweep.mat'), ...
    'CFG', 'MODEL_SPECS', 'T_conditions', 'T_all_pred', ...
    'T_all_roi', 'T_all_overall', 'T_snr_roi', 'T_snr_overall', '-v7.3');

%% Summary figures

if CFG.MAP05.FIG.SaveSummaryFigures
    plot_summary_figures(T_all_roi, T_all_overall, CFG, fig_dir);
end

fprintf('\nTest 12 Level 05 map prediction sweep complete.\n');
fprintf('Analysis folder:\n%s\n', analysis_dir);
fprintf('\nCenter ROI metrics preview.\n');
disp(sortrows(T_all_roi(:, {'model_name', 'SIM_f0', 'REQ_M', ...
    'SIM_cs_bg', 'SIM_SNR_dB', 'SNR_label', 'aperture_label', ...
    'MAPE_pct', 'CoV_pct'}), {'model_name', 'MAPE_pct'}));
fprintf('\nSNR summary, center ROI.\n');
disp(T_snr_roi(:, {'model_name', 'SNR_label', 'n_conditions', ...
    'MAPE_median_pct', 'MAPE_p95_pct', 'CoV_median_pct', ...
    'HighError_gt10_pct', 'HighError_gt20_pct'}));
...
requested_names = string(CFG.MAP05.MODEL.ModelName);
if CFG.MAP05.MODEL.IncludeLocalOnly
...
for i = 1:numel(requested_names)
    try
        [MODEL_i, INFO_i, file_i] = adaptive_req.analysis.load_q_model_deployment( ...
            ...
            'FeatureSet', CFG.MAP05.MODEL.FeatureSet, ...
            'ModelType', CFG.MAP05.MODEL.ModelType);
    catch ME
        error(['Could not load the deployment model for %s | %s | %s.\n', ...
            ...
            requested_names(i), CFG.MAP05.MODEL.FeatureSet, ...
            CFG.MAP05.MODEL.ModelType, deployment_dir, ME.message);
    end
...
for ai = 1:numel(CFG.MAP05.ConeHalfAngleDeg)
    for si = 1:numel(CFG.MAP05.SNR_list)
        for ci = 1:numel(CFG.MAP05.cs_bg_list)
            for mi = 1:numel(CFG.MAP05.M_list)
                for fi = 1:numel(CFG.MAP05.f_list)
                    idx = idx + 1;
                    rows(idx).condition_id = idx;
                    rows(idx).SIM_f0 = CFG.MAP05.f_list(fi);
                    rows(idx).REQ_M = CFG.MAP05.M_list(mi);
                    rows(idx).SIM_cs_bg = CFG.MAP05.cs_bg_list(ci);
                    rows(idx).SIM_SNR_dB = CFG.MAP05.SNR_list(si);
                    rows(idx).SNR_label = snr_label(CFG.MAP05.SNR_list(si));
                    rows(idx).aperture_idx = ai;
                    rows(idx).aperture_label = CFG.MAP05.aperture_labels(ai);
                    rows(idx).ConeHalfAngleDeg = CFG.MAP05.ConeHalfAngleDeg(ai);
                end
            end
        end
    end
...
OUT = adaptive_req.estimators.req_estimator_map( ...
    ...
    'StepX', CFG.MAP05.REQ.StepX, ...
    'StepZ', CFG.MAP05.REQ.StepZ, ...
    'EdgeMode', CFG.MAP05.REQ.EdgeMode, ...
...
T.condition_label = repmat(string(sprintf( ...
    'f%d_M%g_cs%g_SNR%s_%s', cond.SIM_f0, cond.REQ_M, ...
    cond.SIM_cs_bg, cond.SNR_label, cond.aperture_label)), height(T), 1);
...
T.SIM_SNR_dB = cond.SIM_SNR_dB * ones(height(T), 1);
T.SNR_label = repmat(string(cond.SNR_label), height(T), 1);
...
T.roi_name = classify_center_roi(T.x_center_m, T.z_center_m, CFG.MAP05.ROI);
...
vars = ["condition_id", "condition_label", "step_idx", ...
    ...
    "SIM_WaveModel", "SIM_Nwaves", "SIM_SNR_dB", "SNR_label", ...
    "REQ_M", "REQ_cs_guess", ...
...
[G, T_roi] = findgroups(T(:, {'condition_id', 'model_name', ...
    ...
    'SIM_cs_bg', 'SIM_SNR_dB', 'SNR_label', ...
...
function T_snr = summarize_snr_metrics(T)
...
[G, T_snr] = findgroups(T(:, {'model_name', 'feature_set', ...
    'model_type', 'roi_name', 'SIM_SNR_dB', 'SNR_label'}));
T_snr.n_conditions = splitapply(@numel, T.MAPE_pct, G);
T_snr.MAPE_mean_pct = splitapply(@(x) mean(x, 'omitnan'), T.MAPE_pct, G);
T_snr.MAPE_median_pct = splitapply(@(x) median(x, 'omitnan'), T.MAPE_pct, G);
T_snr.MAPE_p95_pct = splitapply(@(x) prctile_finite(x, 95), T.MAPE_pct, G);
T_snr.RMSE_mean_pct = splitapply(@(x) mean(x, 'omitnan'), T.RMSE_pct, G);
T_snr.bias_mean_pct = splitapply(@(x) mean(x, 'omitnan'), T.bias_pct, G);
T_snr.CoV_mean_pct = splitapply(@(x) mean(x, 'omitnan'), T.CoV_pct, G);
T_snr.CoV_median_pct = splitapply(@(x) median(x, 'omitnan'), T.CoV_pct, G);
T_snr.HighError_gt10_pct = splitapply(@(x) 100 * mean(x > 10, 'omitnan'), ...
    T.MAPE_pct, G);
T_snr.HighError_gt20_pct = splitapply(@(x) 100 * mean(x > 20, 'omitnan'), ...
    T.MAPE_pct, G);

T_snr = sortrows(T_snr, {'model_name', 'SIM_SNR_dB'}, ...
    {'ascend', 'descend'});
...
for mi = 1:numel(models)
    ...
    base = sprintf('level12_level05_cond%03d_%s_%s_SNR%s_%s', ...
        cond.condition_id, sanitize_filename(models(mi)), ...
        sanitize_filename(Tm.feature_set(1)), ...
        sanitize_filename(cond.SNR_label), ...
    ...
    colorbar(ax);
    %clim(ax, sws_color_limits(CFG.MAP05.cs_bg_list));
    title(ax, 'Predicted SWS', 'FontSize', 12, 'FontWeight', 'normal');
    xlabel(ax, 'x (cm)');
    ylabel(ax, 'z (cm)');
    draw_roi(ax, CFG.MAP05.ROI);
    ...
    set(ax, 'YDir', 'normal', 'FontSize', 11);
    colorbar(ax);
    %clim(ax, CFG.MAP05.FIG.ErrorLimitsPct);
    ...
    title(tl, sprintf('%s | %s | f=%d Hz | M=%g | c_s=%g | SNR=%s | %s', ...
        models(mi), Tm.feature_set(1), cond.SIM_f0, cond.REQ_M, ...
        cond.SIM_cs_bg, cond.SNR_label, cond.aperture_label), ...
...
plot_metric_heatmaps(T_roi, 'MAPE_pct', CFG, fig_dir, ...
    'level12_level05_center_roi_mape_heatmaps.png', ...
    'Center ROI MAPE (%)');
plot_metric_heatmaps(T_roi, 'CoV_pct', CFG, fig_dir, ...
    'level12_level05_center_roi_cov_heatmaps.png', ...
    'Center ROI CoV (%)');
plot_metric_heatmaps(T_all, 'MAPE_pct', CFG, fig_dir, ...
    'level12_level05_overall_mape_heatmaps.png', ...
    'Overall map MAPE (%)');

plot_metric_box(T_roi, 'MAPE_pct', fig_dir, ...
    'level12_level05_center_roi_mape_box_by_params.png');
plot_metric_box(T_roi, 'CoV_pct', fig_dir, ...
    'level12_level05_center_roi_cov_box_by_params.png');
plot_frequency_curves(T_roi, fig_dir);
plot_snr_diagnostic_figures(T_roi, T_all, fig_dir);

end

function plot_snr_diagnostic_figures(T_roi, T_all, fig_dir)

if isempty(T_roi)
    return;
end

plot_snr_metric_curves(T_roi, 'MAPE_pct', fig_dir, ...
    'level12_level05_center_roi_mape_vs_snr.png', ...
    'Center ROI MAPE vs SNR');
plot_snr_metric_curves(T_roi, 'CoV_pct', fig_dir, ...
    'level12_level05_center_roi_cov_vs_snr.png', ...
    'Center ROI CoV vs SNR');
plot_snr_metric_curves(T_all, 'MAPE_pct', fig_dir, ...
    'level12_level05_overall_mape_vs_snr.png', ...
    'Overall map MAPE vs SNR');

plot_snr_metric_box(T_roi, 'MAPE_pct', fig_dir, ...
    'level12_level05_center_roi_mape_box_by_snr.png');
plot_snr_metric_box(T_roi, 'CoV_pct', fig_dir, ...
    'level12_level05_center_roi_cov_box_by_snr.png');
plot_snr_m_eff_heatmap(T_roi, fig_dir);
plot_snr_aperture_curves(T_roi, fig_dir);

end

function plot_snr_metric_curves(T, metric_var, fig_dir, file_name, title_text)

models = unique(T.model_name, 'stable');
fig = figure('Color', 'w', 'Units', 'centimeters', 'Position', [2 2 24 12]);
tiledlayout(numel(models), 1, 'TileSpacing', 'compact', 'Padding', 'compact');

for mi = 1:numel(models)
    ax = nexttile;
    Ti = T(T.model_name == models(mi), :);
    [G, Tg] = findgroups(Ti(:, {'SNR_label', 'SIM_SNR_dB'}));
    Tg.metric_median = splitapply(@(x) median(x, 'omitnan'), ...
        Ti.(metric_var), G);
    Tg.metric_p25 = splitapply(@(x) prctile_finite(x, 25), Ti.(metric_var), G);
    Tg.metric_p75 = splitapply(@(x) prctile_finite(x, 75), Ti.(metric_var), G);
    Tg = sortrows(Tg, 'SIM_SNR_dB', 'descend');

    x = 1:height(Tg);
    errorbar(ax, x, Tg.metric_median, ...
        Tg.metric_median - Tg.metric_p25, ...
        Tg.metric_p75 - Tg.metric_median, '-o', 'LineWidth', 1.5);
    xticks(ax, x);
    xticklabels(ax, Tg.SNR_label);
    xlabel(ax, 'SNR (dB)');
    ylabel(ax, strrep(metric_var, '_', '\_'));
    title(ax, sprintf('%s | %s', title_text, models(mi)), ...
        'Interpreter', 'none');
    grid(ax, 'on');
end
...
function plot_snr_metric_box(T, metric_var, fig_dir, file_name)

fig = figure('Color', 'w', 'Units', 'centimeters', 'Position', [2 2 22 10]);
boxchart(snr_categorical(T.SNR_label), T.(metric_var), ...
    'GroupByColor', categorical(T.aperture_label));
xlabel('SNR (dB)');
ylabel(strrep(metric_var, '_', '\_'));
title(sprintf('%s by SNR and aperture', strrep(metric_var, '_', '\_')));
grid on;
...
function plot_snr_m_eff_heatmap(T, fig_dir)
...
for mi = 1:numel(models)
    for ai = 1:numel(apertures)
        ...
        snr_values = unique(Ti.SIM_SNR_dB, 'stable');
        snr_values = sort(snr_values, 'descend');
        M_values = unique(Ti.REQ_M, 'stable');
        Z = nan(numel(M_values), numel(snr_values));

        for ii = 1:numel(M_values)
            for jj = 1:numel(snr_values)
                idx = Ti.REQ_M == M_values(ii) & ...
                    Ti.SIM_SNR_dB == snr_values(jj);
                if any(idx)
                    Z(ii, jj) = median(Ti.MAPE_pct(idx), 'omitnan');
                end
            end
        end
        ...
        fig = figure('Color', 'w', 'Units', 'centimeters', ...
            'Position', [2 2 14 10]);
        imagesc(1:numel(snr_values), M_values, Z);
        set(gca, 'YDir', 'normal');
        xticks(1:numel(snr_values));
        xticklabels(string(arrayfun(@snr_label, snr_values, ...
            'UniformOutput', false)));
        xlabel('SNR (dB)');
        ylabel('REQ M');
        title(sprintf('Median center ROI MAPE | %s | %s', ...
            models(mi), apertures(ai)), 'Interpreter', 'none');
        colorbar;

        exportgraphics(fig, fullfile(fig_dir, ...
            sprintf('level12_level05_snr_M_mape_heatmap_%s_%s.png', ...
            sanitize_filename(models(mi)), sanitize_filename(apertures(ai)))), ...
            'Resolution', 250, 'BackgroundColor', 'white');
        close(fig);
    end
end
...
function plot_snr_aperture_curves(T, fig_dir)
...
for mi = 1:numel(models)
    ax = nexttile;
    Ti = T(T.model_name == models(mi), :);
    apertures = unique(Ti.aperture_label, 'stable');
    hold(ax, 'on');
    for ai = 1:numel(apertures)
        Ta = Ti(Ti.aperture_label == apertures(ai), :);
        [G, Tg] = findgroups(Ta(:, {'SNR_label', 'SIM_SNR_dB'}));
        Tg.MAPE_median = splitapply(@(x) median(x, 'omitnan'), ...
            Ta.MAPE_pct, G);
        Tg = sortrows(Tg, 'SIM_SNR_dB', 'descend');
        plot(ax, 1:height(Tg), Tg.MAPE_median, '-o', ...
            'LineWidth', 1.5, 'DisplayName', apertures(ai));
    end
    labels = snr_labels_from_table(Ti);
    xticks(ax, 1:numel(labels));
    xticklabels(ax, labels);
    xlabel(ax, 'SNR (dB)');
    ylabel(ax, 'Median MAPE (%)');
    title(ax, sprintf('SNR degradation by aperture | %s', models(mi)), ...
        'Interpreter', 'none');
    grid(ax, 'on');
...
exportgraphics(fig, fullfile(fig_dir, ...
    'level12_level05_snr_degradation_by_aperture.png'), ...
    'Resolution', 250, 'BackgroundColor', 'white');
close(fig);

end

function labels = snr_labels_from_table(T)

S = unique(T(:, {'SIM_SNR_dB', 'SNR_label'}), 'rows', 'stable');
S = sortrows(S, 'SIM_SNR_dB', 'descend');
labels = S.SNR_label;

end

function c = snr_categorical(labels)

order = ["Inf", "40", "30", "20", "15", "10"];
c = categorical(string(labels), order, order, 'Ordinal', true);
...
exportgraphics(fig, fullfile(fig_dir, ...
    'level12_level05_center_roi_mape_frequency_curves.png'), ...
    'Resolution', 250, 'BackgroundColor', 'white');
close(fig);
